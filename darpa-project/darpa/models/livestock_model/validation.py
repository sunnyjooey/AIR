...
class ValidationContext(object):
    ...
    def is_arg_complete(self, key, require=False):
        ...
        if value in ("", None) or self.limit_to not in (key, None):
            if require:
                self.warn(
                    "Parameter is required but is missing or has no value", keys=(key,)
                )
            return False
        return True
...
def invest_validator(validate_func):
    ...
    def _wrapped_validate_func(args, limit_to=None):
        ...
        assert isinstance(limit_to, type(None)) or isinstance(
            limit_to, basestring
        ), "limit_to parameter must be either a string key or None."
        if limit_to is not None:
            assert limit_to in args, 'limit_to key "%s" must exist in args.' % limit_to

        for key, value in args.items():  # noqa: B007
            assert isinstance(key, basestring), "All args keys must be strings."
        ...
        LOGGER.debug("Validation warnings: %s", pprint.pformat(warnings_))

        assert isinstance(
            warnings_, list
        ), "validate function must return a list of 2-tuples."
        for keys_iterable, error_string in warnings_:
            assert isinstance(keys_iterable, collections.Iterable) and not isinstance(
                keys_iterable, basestring
            ), "Keys entry %s must be a non-string iterable" % (keys_iterable)
