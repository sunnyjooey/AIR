"""Code adopted from https://github.com/natcap/rangeland_production"""
import logging
import math
import os
import pickle
...
from .validation import invest_validator
...
def execute(args):
    ...
    for month_index in range(n_months):
        ...
        precip_path = (
            ...
            .replace("<month>", "{:02d}".format(month_i))
        )
        base_align_raster_path_id_map["precip_{}".format(month_index)] = precip_path
        ...
        base_align_raster_path_id_map["EO_index_{}".format(month_index)] = EO_index_path
    ...
    for substring in ["min", "max"]:
        ...
        for month_i in temperature_month_set:
            monthly_temp_path = args["%s_temp_path_pattern" % substring].replace(
                "<month>", "%.2d" % month_i
            )
            base_align_raster_path_id_map[
                "%s_temp_%d" % (substring, month_i)
            ] = monthly_temp_path
    ...
    for soil_type in SOIL_TYPE_LIST:
        base_align_raster_path_id_map[soil_type] = args[
            "%s_proportion_path" % soil_type
        ]
    ...
    # identifier
    base_align_raster_path_id_map["site_index"] = args["site_param_spatial_index_path"]
    n_bands = pygeoprocessing.get_raster_info(args["site_param_spatial_index_path"])[
        "n_bands"
    ]
    if n_bands > 1:
        raise ValueError("Site spatial index raster must contain only one band")
    ...
    site_index_set = set()
    for offset_map, raster_block in pygeoprocessing.iterblocks(  # noqa: B007
    ...
    site_param_table = build_lookup_from_csv(args["site_param_table"], "site")
    missing_site_index_list = list(site_index_set.difference(site_param_table.keys()))
    ...
    pft_basename = os.path.basename(args["veg_spatial_composition_path_pattern"])

    files = [f for f in os.listdir(pft_dir) if os.path.isfile(os.path.join(pft_dir, f))]

    pft_regex = re.compile(pft_basename.replace("<PFT>", r"(\d+)"))
    pft_matches = [m for m in [pft_regex.search(f) for f in files] if m is not None]
    # try:
    pft_id_set = set([int(m.group(1)) for m in pft_matches])
    # except IndexError:
    # pft_id_set = set([int(m.group(0).split('.')[0].replace('pft', '')) for m in pft_matches])
    ...
    veg_trait_table = build_lookup_from_csv(args["veg_trait_path"], "PFT")
    ...
    frtcindx_set = set([pft_i["frtcindx"] for pft_i in veg_trait_table.values()])
    ...
    for pft_i in pft_id_set:
        for sv in _PFT_STATE_VARIABLES:
            pft_sv_dict["{}_{}_path".format(sv, pft_i)] = "{}_{}.tif".format(sv, pft_i)
    ...
    target_pixel_size = pygeoprocessing.get_raster_info(
        base_align_raster_path_id_map["precip_0"]
    )["pixel_size"]
    LOGGER.info("pixel size of aligned inputs: %s", target_pixel_size)
    ...
    aligned_raster_dir = os.path.join(args["workspace_dir"], "aligned_inputs")
    ...
    aligned_inputs = dict(
        [
            (
                key,
                os.path.join(aligned_raster_dir, "aligned_%s" % os.path.basename(path)),
            )
    ...
    source_input_path_list = [
        base_align_raster_path_id_map[k]
        for k in sorted(base_align_raster_path_id_map.keys())
    ]
    aligned_input_path_list = [aligned_inputs[k] for k in sorted(aligned_inputs.keys())]
    ...
    file_suffix = make_suffix_string(args, "results_suffix")
    ...
    pygeoprocessing.rasterize(
        args["animal_grazing_areas_path"],
        aligned_inputs["animal_index"],
    ...
    if initial_conditions_dir:
        ...
        if len(state_var_nodata) > 1:
            raise ValueError("Initial state variable rasters contain >1 nodata value")
        global _SV_NODATA
        _SV_NODATA = list(state_var_nodata)[0]

        # align initial values with inputs
        initial_path_list = [aligned_inputs["precip_0"]] + [
            resample_initial_path_map[key]
            for key in sorted(resample_initial_path_map.keys())
        ]
        aligned_initial_path_list = [
            os.path.join(PROCESSING_DIR, "aligned_input_template.tif")
        ] + [
            os.path.join(sv_dir, os.path.basename(resample_initial_path_map[key]))
            for key in sorted(resample_initial_path_map.keys())
        ]
        pygeoprocessing.align_and_resize_raster_stack(
            initial_path_list,
        ...
        try:
            site_initial_conditions_table = build_lookup_from_csv(
        ...
        try:
            pft_initial_conditions_table = build_lookup_from_csv(
    ...
    # the simulation
    persist_param_dir = os.path.join(args["workspace_dir"], "intermediate_parameters")
    make_directories([persist_param_dir])
    pp_reg = build_file_registry(
    ...
    freer_parameter_df = pandas.DataFrame.from_dict(_FREER_PARAM_DICT, orient="index")
    ...
    for animal_id in animal_trait_table.keys():
        revised_animal_trait_dict = calc_max_intake(animal_trait_table[animal_id])
    ...
    _structural_ratios(aligned_inputs["site_index"], site_param_table, sv_reg, pp_reg)

    # make yearly directory for values that are updated every twelve months
    year_dir = tempfile.mkdtemp(dir=PROCESSING_DIR)
    year_reg = dict(
        [(key, os.path.join(year_dir, path)) for key, path in _YEARLY_FILES.items()]
    )
    for pft_i in pft_id_set:
    ...
    for pft_i in pft_id_set:
        for val in _PFT_INTERMEDIATE_VALUES:
            month_reg["{}_{}".format(val, pft_i)] = os.path.join(
    ...
    provisional_sv_reg = build_file_registry(
        [
            (_SITE_STATE_VARIABLE_FILES, provisional_sv_dir),
            (pft_sv_dict, provisional_sv_dir),
        ],
        file_suffix,
    )
    ...
    for month_index in range(n_months):
        ...
        make_directories([sv_dir])
        sv_reg = build_file_registry(
            [(_SITE_STATE_VARIABLE_FILES, sv_dir), (pft_sv_dict, sv_dir)], file_suffix
        )
        ...
        _shoot_senescence(
            pft_id_set, veg_trait_table, prev_sv_reg, month_reg, current_month, sv_reg
        )
    ...
    summary_shp_path = os.path.join(summary_output_dir, "grazing_areas_results_rpm.shp")
    ...
    if delete_sv_folders:
        for month_index in range(-1, n_months):
            shutil.rmtree(
                os.path.join(args["workspace_dir"], "state_variables_m%d" % month_index)
            )


def raster_multiplication(
    raster1, raster1_nodata, raster2, raster2_nodata, target_path, target_path_nodata
):
    ...
    def raster_multiply_op(raster1, raster2):
        ...
        valid_mask = (~numpy.isclose(raster1, raster1_nodata)) & (
...
def raster_division(
    ...
    def raster_divide_op(raster1, raster2):
        ...
        result = numpy.empty(raster1.shape, dtype=numpy.float32)
        result[:] = target_path_nodata
        error_mask = (raster1 != 0) & (raster2 == 0.0) & valid_mask
        zero_mask = (raster1 == 0.0) & (raster2 == 0.0) & valid_mask
        nonzero_mask = (raster2 != 0.0) & valid_mask
...
def raster_list_sum(
    raster_list, input_nodata, target_path, target_nodata, nodata_remove=False
):
...
def raster_difference(
    ...
    if nodata_remove:
        pygeoprocessing.raster_calculator(
            [(path, 1) for path in [raster1, raster2]],
            raster_difference_op_nodata_remove,
            target_path,
            gdal.GDT_Float32,
            target_nodata,
        )
...
def reclassify_nodata(target_path, new_nodata_value):
    ...
    previous_nodata_value = pygeoprocessing.get_raster_info(target_path)["nodata"][0]

    pygeoprocessing.raster_calculator(
        [(temp_path, 1)], reclassify_op, target_path, gdal.GDT_Float32, new_nodata_value
    )
...
def _check_pft_fractional_cover_sum(aligned_inputs, pft_id_set):
    ...
    with tempfile.NamedTemporaryFile(
        prefix="cover_sum", dir=PROCESSING_DIR
    ) as cover_sum_temp_file:
        cover_sum_path = cover_sum_temp_file.name
    with tempfile.NamedTemporaryFile(
        prefix="operand_temp", dir=PROCESSING_DIR
    ) as operand_temp_file:
    ...
    if max_cover > 1:
        raise ValueError("Fractional cover across plant functional types exceeds 1")
...
def initial_conditions_from_tables(
    ...
    def full_masked(pft_cover, fill_val):
        ...
        with warnings.catch_warnings():
            ...
            valid_mask = (~numpy.isclose(pft_cover, _SV_NODATA)) & (pft_cover > 0)
    ...
    for site_code in site_initial_conditions_table.keys():
        ...
        if missing_site_state_var:
            raise ValueError(
                ...
                + "\n\t".join(missing_site_state_var)
            )
    for sv_key, basename in _SITE_STATE_VARIABLE_FILES.items():
        state_var = sv_key[:-5]
        site_to_val = dict(
            [
                (site_code, float(table[state_var]))
                for (site_code, table) in site_initial_conditions_table.items()
            ]
        )
        target_path = os.path.join(sv_dir, basename)
        initial_sv_reg[sv_key] = target_path
        pygeoprocessing.reclassify_raster(
            (aligned_inputs["site_index"], 1),
            site_to_val,
            target_path,
            gdal.GDT_Float32,
            _SV_NODATA,
    ...
    for pft_i in pft_id_set:
        ...
        for state_var in _PFT_STATE_VARIABLES:
            ...
            target_path = os.path.join(sv_dir, "{}_{}.tif".format(state_var, pft_i))
...
def _calc_ompc(
    som1c_2_path, som2c_2_path, som3c_path, bulkd_path, edepth_path, ompc_path
):
    ...
    def ompc_op(som1c_2, som2c_2, som3c, bulkd, edepth):
        ...
        ompc[valid_mask] = (
            ...
            * 1.724
            / (10000.0 * bulkd[valid_mask] * edepth[valid_mask])
        )
        return ompc
...
def _calc_afiel(sand_path, silt_path, clay_path, ompc_path, bulkd_path, afiel_path):
    ...
    def afiel_op(sand, silt, clay, ompc, bulkd):
        ...
        afiel[valid_mask] = (
            0.3075 * sand[valid_mask]
            + 0.5886 * silt[valid_mask]
            + 0.8039 * clay[valid_mask]
            + 2.208e-03 * ompc[valid_mask]
    ...
    pygeoprocessing.raster_calculator(
        [
            (path, 1)
            for path in [sand_path, silt_path, clay_path, ompc_path, bulkd_path]
        ],
        afiel_op,
        afiel_path,
        gdal.GDT_Float32,
...
def _calc_awilt(sand_path, silt_path, clay_path, ompc_path, bulkd_path, awilt_path):
    ...
    def awilt_op(sand, silt, clay, ompc, bulkd):
        ...
        awilt[valid_mask] = (
            -0.0059 * sand[valid_mask]
            + 0.1142 * silt[valid_mask]
            + 0.5766 * clay[valid_mask]
            + 2.228e-03 * ompc[valid_mask]
...
def _afiel_awilt(
    ...
    site_to_edepth = dict(
        [
            (site_code, float(table["edepth"]))
            for (site_code, table) in site_param_table.items()
        ]
    )

    pygeoprocessing.reclassify_raster(
        (site_index_path, 1), site_to_edepth, edepth_path, gdal.GDT_Float32, _IC_NODATA
    )

    # estimate total soil organic matter
    _calc_ompc(
        som1c_2_path, som2c_2_path, som3c_path, bulk_d_path, edepth_path, ompc_path
    )
    ...
    for lyr in range(1, 10):
        ...
        _calc_afiel(sand_path, silt_path, clay_path, ompc_path, bulk_d_path, afiel_path)
        _calc_awilt(sand_path, silt_path, clay_path, ompc_path, bulk_d_path, awilt_path)
...
def _persistent_params(site_index_path, site_param_table, sand_path, clay_path, pp_reg):
    ...
    for val in [
        ...
        site_to_val = dict(
            [
                (site_code, float(table[val]))
                for (site_code, table) in site_param_table.items()
            ]
        )
        pygeoprocessing.reclassify_raster(
            (site_index_path, 1), site_to_val, target_path, gdal.GDT_Float32, _IC_NODATA
        )
    ...
    pygeoprocessing.raster_calculator(
        [(path, 1) for path in [pp_reg["afiel_1_path"], pp_reg["awilt_1_path"]]],
        calc_wc,
        pp_reg["wc_path"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    ...
    pygeoprocessing.raster_calculator(
        [
            (path, 1)
            for path in [param_val_dict["peftxa"], param_val_dict["peftxb"], sand_path]
        ],
        calc_eftext,
        pp_reg["eftext_path"],
        gdal.GDT_Float32,
    ...
    pygeoprocessing.raster_calculator(
        [
            (path, 1)
            for path in [
                param_val_dict["ps1s3_1"],
                param_val_dict["ps1s3_2"],
                clay_path,
            ]
        ],
        calc_fps1s3,
    ...
    pygeoprocessing.raster_calculator(
        [
            (path, 1)
            for path in [
                param_val_dict["ps2s3_1"],
                param_val_dict["ps2s3_2"],
                clay_path,
            ]
        ],
        calc_fps2s3,
    ...
    pygeoprocessing.raster_calculator(
        [
            (path, 1)
            for path in [
                param_val_dict["omlech_1"],
                param_val_dict["omlech_2"],
                sand_path,
            ]
        ],
        calc_orglch,
    ...
    def calc_vlossg(vlossg_param, clay):
        ...
        """
        valid_mask = (vlossg_param != _IC_NODATA) & (~numpy.isclose(clay, clay_nodata))
...
def _aboveground_ratio(anps, tca, pcemic_1, pcemic_2, pcemic_3):
    ...
    cemicb[:] = _IC_NODATA
    cemicb[valid_mask] = (pcemic_2[valid_mask] - pcemic_1[valid_mask]) / pcemic_3[
        valid_mask
    ]
    ...
    decompose_mask = (tca > 0.0) & valid_mask
...
def _belowground_ratio(aminrl, varat_1_iel, varat_2_iel, varat_3_iel):
    ...
    bgdrat[valid_mask] = (1.0 - aminrl[valid_mask] / varat_3_iel[valid_mask]) * (
...
def _structural_ratios(site_index_path, site_param_table, sv_reg, pp_reg):
    ...
    for iel in [1, 2]:
        for val in [
            ...
            site_to_val = dict(
                [
                    (site_code, float(table["{}_{}".format(val, iel)]))
                    for (site_code, table) in site_param_table.items()
                ]
            )
            pygeoprocessing.reclassify_raster(
    ...
    def calc_rnewas_som2(
        ...
        rnewas2[valid_mask] = numpy.maximum(rnewas2[valid_mask], rad1p_3[valid_mask])
        return rnewas2
    ...
    for iel in [1, 2]:
        ...
        pygeoprocessing.raster_calculator(
            ...
            calc_rnewas_som2,
            pp_reg["rnewas_{}_2_path".format(iel)],
            gdal.GDT_Float32,
            _TARGET_NODATA,
        )
        ...
        site_to_varat1_1 = dict(
            [
                (site_code, float(table["varat1_1_{}".format(iel)]))
                for (site_code, table) in site_param_table.items()
            ]
        )
        pygeoprocessing.reclassify_raster(
        ...
        site_to_varat22_1 = dict(
            [
                (site_code, float(table["varat22_1_{}".format(iel)]))
                for (site_code, table) in site_param_table.items()
            ]
        )
        pygeoprocessing.reclassify_raster(
...
def _yearly_tasks(
    aligned_inputs, site_param_table, veg_trait_table, month_index, pft_id_set, year_reg
):
    ...
    def calc_base_N_dep(epnfa_1, epnfa_2, prcann):
        ...
        baseNdep[valid_mask] = epnfa_1[valid_mask] + (
            epnfa_2[valid_mask] * numpy.minimum(prcann[valid_mask], 80.0)
        )
        baseNdep[baseNdep < 0] = 0.0
        return baseNdep
    ...
    def calc_pltlig(fligni_1_lyr, fligni_2_lyr, prcann):
        ...
        pltlig[valid_mask] = (
            fligni_1_lyr[valid_mask] + fligni_2_lyr[valid_mask] * prcann[valid_mask]
        )
    ...
    while len(annual_precip_rasters) < 12:
        ...
        try:
            annual_precip_rasters.append(aligned_inputs["precip_%d" % precip_month])
        except KeyError:
            continue
    ...
    for val in ["fligni_1_1", "fligni_2_1", "fligni_1_2", "fligni_2_2"]:
        for pft_i in pft_id_set:
            target_path = os.path.join(temp_dir, "{}_{}.tif".format(val, pft_i))
    ...
    pygeoprocessing.raster_calculator(
        [
            (path, 1)
            for path in [
                param_val_dict["epnfa_1"],
                param_val_dict["epnfa_2"],
                year_reg["annual_precip_path"],
            ]
        ],
        calc_base_N_dep,
    ...
    for pft_i in pft_id_set:
        ...
        pygeoprocessing.raster_calculator(
            ...
            calc_pltlig,
            year_reg["pltlig_above_{}".format(pft_i)],
            gdal.GDT_Float32,
            _TARGET_NODATA,
        )
        ...
        pygeoprocessing.raster_calculator(
            ...
            calc_pltlig,
            year_reg["pltlig_below_{}".format(pft_i)],
            gdal.GDT_Float32,
            _TARGET_NODATA,
        )
...
def calc_latitude(template_raster, latitude_raster_path):
    ...
    pygeoprocessing.new_raster_from_base(
        template_raster, latitude_raster_path, gdal.GDT_Float32, [_IC_NODATA]
    )
    latitude_raster = gdal.OpenEx(latitude_raster_path, gdal.OF_RASTER | gdal.GA_Update)
    ...
    for offset_map, raster_block in pygeoprocessing.iterblocks((template_raster, 1)):
        ...
        valid_mask = raster_block != base_raster_info["nodata"]  # noqa: F841
...
def _calc_daylength(template_raster, month, daylength_path):
    ...
    def daylength(month):
        def _daylength(latitude):
            ...
            jday_list = [1, 32, 61, 92, 122, 153, 183, 214, 245, 275, 306, 337]
        ...
        def _shwave(latitude):
            ...
            solrad = (
                ...
                * (
                    ...
                    + numpy.cos(rlatitude) * numpy.cos(declin) * numpy.sin(ahou)
                )
            )
...
def _reference_evapotranspiration(
    max_temp_path, min_temp_path, shwave_path, fwloss_4_path, pevap_path
):
    ...
    def _calc_pevap(max_temp, min_temp, shwave, fwloss_4):
        ...
        in1 = const1 * (tmean[valid_mask] + const2)  # noqa: F841
        in2 = numpy.sqrt(trange[valid_mask])  # noqa: F841
        in3 = shwave[valid_mask] / langleys2watts  # noqa: F841
        daypet[valid_mask] = (
            const1
            * (tmean[valid_mask] + const2)
    ...
    maxtmp_nodata = pygeoprocessing.get_raster_info(max_temp_path)["nodata"][0]
    mintmp_nodata = pygeoprocessing.get_raster_info(min_temp_path)["nodata"][0]
    pygeoprocessing.raster_calculator(
        [
            (path, 1)
            for path in [max_temp_path, min_temp_path, shwave_path, fwloss_4_path]
        ],
        _calc_pevap,
        pevap_path,
        gdal.GDT_Float32,
...
def _potential_production(
    ...
    def calc_ctemp(aglivc, pmxbio, maxtmp, pmxtmp, mintmp, pmntmp):
        ...
        tmxs[:] = _IC_NODATA
        tmxs[valid_mask] = maxtmp[valid_mask] + (
            (25.4 / (1.0 + 18.0 * numpy.exp(-0.20 * maxtmp[valid_mask])))
        ...
        tmns[:] = _IC_NODATA
        tmns[valid_mask] = mintmp[valid_mask] + (
        ...
        ctemp[:] = _IC_NODATA
        ctemp[valid_mask] = (tmxs[valid_mask] + tmns[valid_mask]) / 2.0
        return ctemp
    ...
    def calc_potprd(mintmp, maxtmp, ctemp, ppdf_1, ppdf_2, ppdf_3, ppdf_4):
        ...
        frac[:] = _TARGET_NODATA
        frac[valid_mask] = (ppdf_2[valid_mask] - ctemp[valid_mask]) / (
            ppdf_2[valid_mask] - ppdf_1[valid_mask]
        )
        avg_tmp = numpy.empty(ctemp.shape, dtype=numpy.float32)
        avg_tmp[valid_mask] = (mintmp[valid_mask] + maxtmp[valid_mask]) / 2.0
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            grow_mask = (avg_tmp > 0) & (frac > 0) & valid_mask
        potprd = numpy.empty(ctemp.shape, dtype=numpy.float32)
        potprd[:] = _TARGET_NODATA
        potprd[valid_mask] = 0.0
        potprd[grow_mask] = numpy.exp(
            (ppdf_3[grow_mask] / ppdf_4[grow_mask])
            * (1.0 - numpy.power(frac[grow_mask], ppdf_4[grow_mask]))
        ) * numpy.power(frac[grow_mask], ppdf_3[grow_mask])
        return potprd

    def calc_h2ogef_1(pevap, avh2o_1, precip, wc, pprpts_1, pprpts_2, pprpts_3):
        ...
        intcpt = pprpts_1[valid_mask] + (pprpts_2[valid_mask] * wc[valid_mask])
        slope = 1.0 / (pprpts_3[valid_mask] - intcpt)

        h2ogef_1 = numpy.empty(pevap.shape, dtype=numpy.float32)
        h2ogef_1[:] = _TARGET_NODATA
        h2ogef_1[valid_mask] = 1.0 + slope * (
            h2ogef_prior[valid_mask] - pprpts_3[valid_mask]
        )

        h2ogef_1[valid_mask] = numpy.clip(h2ogef_1[valid_mask], 0.01, 1.0)
        return h2ogef_1
    ...
    def calc_biof(sum_stdedc, sum_aglivc, strucc_1, pmxbio, biok5):
        ...
        bioc[valid_mask] = numpy.where(
            ((sum_stdedc[valid_mask] + 0.1 * strucc_1[valid_mask]) <= 0.0),
            0.01,
            (sum_stdedc[valid_mask] + 0.1 * strucc_1[valid_mask]),
        )
        bioc[valid_mask] = numpy.where(
            (bioc[valid_mask] > pmxbio[valid_mask]),
            pmxbio[valid_mask],
            bioc[valid_mask],
        )
    ...
    for pft_i in pft_id_set:
        for val in ["aglivc_weighted", "stdedc_weighted", "potprd", "biof"]:
            temp_val_dict["{}_{}".format(val, pft_i)] = os.path.join(
                temp_dir, "{}_{}.tif".format(val, pft_i)
            )
    ...
    for val in ["ppdf_1", "ppdf_2", "ppdf_3", "ppdf_4", "biok5", "prdx_1"]:
        for pft_i in do_PFT:
            target_path = os.path.join(temp_dir, "{}_{}.tif".format(val, pft_i))
            param_val_dict["{}_{}".format(val, pft_i)] = target_path
            fill_val = veg_trait_table[pft_i][val]
...
def _calc_favail_P(sv_reg, param_val_dict):
    ...
    def favail_P_op(minerl_1_1, favail_4, favail_5, favail_6):
        ...
        favail_P[valid_mask] = numpy.maximum(
            favail_4[valid_mask],
            numpy.minimum(interim[valid_mask], favail_5[valid_mask]),
        )
        return favail_P
...
def _calc_avail_mineral_nutrient(pft_param_dict, sv_reg, iel, target_path):
    ...
    mineral_raster_list = [
        sv_reg["minerl_{}_{}_path".format(lyr, iel)] for lyr in range(1, nlay + 1)
    ]
    raster_list_sum(
        mineral_raster_list, _SV_NODATA, target_path, _TARGET_NODATA, nodata_remove=True
    )
...
def _calc_available_nutrient(
    ...
    def calc_eavail(rictrl, bglivc, riint, availm, favail, crpstg):
        ...
        rimpct[valid_mask] = numpy.where(
            ((rictrl[valid_mask] * bglivc[valid_mask] * 2.5) > 33.0),
            1.0,
            1.0
            - riint[valid_mask]
            * numpy.exp(-rictrl[valid_mask] * bglivc[valid_mask] * 2.5),
        )
    ...
    if iel == 1:
        ...
        pygeoprocessing.raster_calculator(
            [
                (path, 1)
                for path in [eavail_prior_path, param_val_dict["snfxmx_1"], tgprod_path]
            ],
            add_symbiotic_fixed_N,
            eavail_path,
            gdal.GDT_Float32,
            _TARGET_NODATA,
        )
...
def _calc_nutrient_demand(
    biomass_production_path,
    fraction_allocated_to_roots_path,
    cercrp_min_above_path,
    cercrp_min_below_path,
    demand_path,
):
    ...
    def nutrient_demand_op(
        biomass_production, root_fraction, cercrp_min_above, cercrp_min_below
    ):
        ...
        demand_above[valid_mask] = (
            (biomass_production[valid_mask] * (1.0 - root_fraction[valid_mask])) / 2.5
        ) * (1.0 / cercrp_min_above[valid_mask])

        demand_below = numpy.empty(root_fraction.shape, dtype=numpy.float32)
        demand_below[:] = _TARGET_NODATA
        demand_below[valid_mask] = (
            (biomass_production[valid_mask] * (root_fraction[valid_mask])) / 2.5
        ) * (1.0 / cercrp_min_below[valid_mask])

        demand_e = numpy.empty(root_fraction.shape, dtype=numpy.float32)
        demand_e[:] = _TARGET_NODATA
        demand_e[valid_mask] = demand_above[valid_mask] + demand_below[valid_mask]
        return demand_e
...
def calc_provisional_fracrc(
    ...
    rtsh[:] = _TARGET_NODATA
    rtsh[valid_mask] = (
        bgppa[valid_mask] + annual_precip[valid_mask] * bgppb[valid_mask]
    ) / (agppa[valid_mask] + annual_precip[valid_mask] * agppb[valid_mask])
...
def calc_ce_ratios(
    ...
    def calc_above_ratio(pra_1, pra_2, aglivc, biomax):
        ...
        cercrp_above[valid_mask] = numpy.minimum(
            (
                pra_1[valid_mask]
                + (pra_2[valid_mask] - pra_1[valid_mask])
                * 2.5
                * aglivc[valid_mask]
                / biomax[valid_mask]
            ),
    ...
    def calc_below_ratio(prb_1, prb_2, annual_precip):
        ...
        cercrp_below[valid_mask] = prb_1[valid_mask] + (
            prb_2[valid_mask] * annual_precip[valid_mask]
        )
        return cercrp_below

    pygeoprocessing.raster_calculator(
        [(path, 1) for path in [pramn_1_path, pramn_2_path, aglivc_path, biomax_path]],
        calc_above_ratio,
    ...
    pygeoprocessing.raster_calculator(
        [(path, 1) for path in [pramx_1_path, pramx_2_path, aglivc_path, biomax_path]],
        calc_above_ratio,
    ...
    pygeoprocessing.raster_calculator(
        [(path, 1) for path in [prbmn_1_path, prbmn_2_path, annual_precip_path]],
        calc_below_ratio,
    ...
    pygeoprocessing.raster_calculator(
        [(path, 1) for path in [prbmx_1_path, prbmx_2_path, annual_precip_path]],
        calc_below_ratio,
...
def calc_revised_fracrc(
    ...
    def calc_a2drat(totale, demand):
        ...
        """
        valid_mask = (totale != _TARGET_NODATA) & (demand != _TARGET_NODATA)

        a2drat = numpy.empty(totale.shape, dtype=numpy.float32)
        a2drat[:] = _TARGET_NODATA
        demand_mask = (demand > 0) & valid_mask
        a2drat[valid_mask] = 1.0
        a2drat[demand_mask] = numpy.clip(
            totale[demand_mask] / demand[demand_mask], 0.0, 1.0
        )
        return a2drat

    def calc_perennial_fracrc(
        h2ogef, cfrtcw_1, cfrtcw_2, a2drat_1, a2drat_2, cfrtcn_1, cfrtcn_2
    ):
        ...
        h2oeff[valid_mask] = (cfrtcw_2[valid_mask] - cfrtcw_1[valid_mask]) * (
            h2ogef[valid_mask] - 1.0
        ) + cfrtcw_2[valid_mask]

        ntreff_1 = numpy.empty(h2ogef.shape, dtype=numpy.float32)
        ntreff_1[:] = _TARGET_NODATA
        ntreff_1[valid_mask] = (cfrtcn_2[valid_mask] - cfrtcn_1[valid_mask]) * (
            a2drat_1[valid_mask] - 1.0
        ) + cfrtcn_2[valid_mask]

        ntreff_2 = numpy.empty(h2ogef.shape, dtype=numpy.float32)
        ntreff_2[:] = _TARGET_NODATA
        ntreff_1[valid_mask] = (cfrtcn_2[valid_mask] - cfrtcn_1[valid_mask]) * (
            a2drat_2[valid_mask] - 1.0
        ) + cfrtcn_2[valid_mask]

        ntreff = numpy.empty(h2ogef.shape, dtype=numpy.float32)
        ntreff[:] = _TARGET_NODATA
        ntreff[valid_mask] = numpy.maximum(ntreff_1[valid_mask], ntreff_2[valid_mask])

        fracrc_perennial = numpy.empty(h2ogef.shape, dtype=numpy.float32)
...
def grazing_effect_on_aboveground_production(tgprod, fracrc, flgrem, grzeff):
    ...
    agprod_prior[valid_mask] = tgprod[valid_mask] * (1.0 - fracrc[valid_mask])
    ...
    linear_effect[valid_mask] = numpy.maximum(
        (1.0 - (2.21 * flgrem[valid_mask])) * agprod_prior[valid_mask], 0.02
    )

    quadratic_effect = numpy.empty(tgprod.shape, dtype=numpy.float32)
    quadratic_effect[:] = _TARGET_NODATA
    quadratic_effect[valid_mask] = (
        1.0 + 2.6 * flgrem[valid_mask] - (5.83 * (numpy.power(flgrem[valid_mask], 2)))
    ) * agprod_prior[valid_mask]
    quadratic_effect[valid_mask] = numpy.maximum(quadratic_effect[valid_mask], 0.02)
...
def grazing_effect_on_root_shoot(fracrc, flgrem, grzeff, gremb):
    ...
    rtsh_prior[valid_mask] = fracrc[valid_mask] / (1.0 - fracrc[valid_mask])
    ...
    linear_effect[:] = _TARGET_NODATA
    linear_effect[valid_mask] = numpy.maximum(
        1.0 - (flgrem[valid_mask] * gremb[valid_mask]), 0.01
    )
...
def calc_tgprod_final(rtsh, agprod):
    ...
    """
    valid_mask = (rtsh != _TARGET_NODATA) & (agprod != _TARGET_NODATA)
    tgprod = numpy.empty(rtsh.shape, dtype=numpy.float32)
    tgprod[:] = _TARGET_NODATA
    tgprod[valid_mask] = agprod[valid_mask] + (rtsh[valid_mask] * agprod[valid_mask])
    return tgprod


def calc_final_tgprod_rtsh(
    tgprod_pot_prod_path,
    fracrc_path,
    flgrem_path,
    grzeff_path,
    gremb_path,
    ...
    pygeoprocessing.raster_calculator(
        [
            (path, 1)
            for path in [tgprod_pot_prod_path, fracrc_path, flgrem_path, grzeff_path]
        ],
    ...
    pygeoprocessing.raster_calculator(
        [(path, 1) for path in [fracrc_path, flgrem_path, grzeff_path, gremb_path]],
...
def _root_shoot_ratio(
    ...
    for pft_i in pft_id_set:
        ...
        do_growth = (
            current_month != veg_trait_table[pft_i]["senescence_month"]
            and str(current_month) in veg_trait_table[pft_i]["growth_months"]
        )
        if do_growth:
            do_PFT.append(pft_i)
    if not do_PFT:
    ...
    for pft_i in do_PFT:
        ...
        for iel in [1, 2]:
            for val in ["eavail", "demand"]:
                temp_val_dict["{}_{}_{}".format(val, iel, pft_i)] = os.path.join(
...
def _snow(
    ...
    def calc_snow_moisture(return_type):
        ...
        def _calc_snow_moisture(
            ...
            inputs_after_snow[snowfall_mask] = 0.0

            rain_on_snow_mask = (valid_mask) & (tave > 0) & (snow > 0)
            snlq[rain_on_snow_mask] = (
                snlq[rain_on_snow_mask] + precip[rain_on_snow_mask]
            )
            inputs_after_snow[rain_on_snow_mask] = 0.0

            snowtot = numpy.zeros(snow.shape, dtype=numpy.float32)
            snowtot[valid_mask] = numpy.maximum(snow[valid_mask] + snlq[valid_mask], 0)

            evap_mask = valid_mask & (snowtot > 0.0)
            evsnow = numpy.zeros(snow.shape, dtype=numpy.float32)
            evsnow[evap_mask] = numpy.minimum(snowtot[evap_mask], pet[evap_mask] * 0.87)
            ...
            snow_revised[evap_mask] = numpy.maximum(
                snow[evap_mask]
                - evsnow[evap_mask] * (snow[evap_mask] / snowtot[evap_mask]),
                0.0,
            )
            ...
            snlq_revised[evap_mask] = numpy.maximum(
                snlq[evap_mask]
                - evsnow[evap_mask] * (snlq[evap_mask] / snowtot[evap_mask]),
                0.0,
            )
            ...
            pet_revised[evap_mask] = numpy.maximum(
                (pet[evap_mask] - evsnow[evap_mask] / 0.87), 0.0
            )
            ...
            snowmelt[melt_mask] = numpy.clip(
                tmelt_2[melt_mask]
                * (tave[melt_mask] - tmelt_1[melt_mask])
                * shwave[melt_mask],
                0.0,
    ...
    max_temp_nodata = pygeoprocessing.get_raster_info(max_temp_path)[  # noqa: F841
        "nodata"
    ][0]
    min_temp_nodata = pygeoprocessing.get_raster_info(min_temp_path)[  # noqa: F841
        "nodata"
    ][0]
    ...
    pygeoprocessing.raster_calculator(
        [
            ...
            for path in [
                ...
                prev_snlq_path,
                temp_val_dict["pet"],
                param_val_dict["tmelt_1"],
                param_val_dict["tmelt_2"],
                temp_val_dict["shwave"],
            ]
        ],
        calc_snow_moisture("snowmelt"),
        snowmelt_path,
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    ...
    pygeoprocessing.raster_calculator(
        ...
        calc_snow_moisture("inputs_after_snow"),
        inputs_after_snow_path,
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
...
def _calc_aboveground_live_biomass(sum_aglivc, sum_tgprod):
    ...
    """
    valid_mask = (sum_aglivc != _TARGET_NODATA) & (sum_tgprod != _TARGET_NODATA)
    aliv = numpy.empty(sum_aglivc.shape, dtype=numpy.float32)
    aliv[:] = _TARGET_NODATA
    aliv[valid_mask] = sum_aglivc[valid_mask] * 2.5 + (0.25 * sum_tgprod[valid_mask])
    return aliv
...
def _calc_standing_biomass(aliv, sum_stdedc):
    ...
    """
    valid_mask = (aliv != _TARGET_NODATA) & (sum_stdedc != _TARGET_NODATA)
    sd = numpy.empty(aliv.shape, dtype=numpy.float32)
    sd[:] = _TARGET_NODATA
    sd[valid_mask] = numpy.minimum(
        aliv[valid_mask] + (sum_stdedc[valid_mask] * 2.5), 800.0
    )
    return sd
...
def subtract_surface_losses(return_type):
    ...
    def _subtract_surface_losses(
        inputs_after_snow, fracro, precro, snow, alit, sd, fwloss_1, fwloss_2, pet_rem
    ):
        ...
        runoff[valid_mask] = numpy.maximum(
            fracro[valid_mask] * (inputs_after_snow[valid_mask] - precro[valid_mask]),
            0.0,
        )
        inputs_after_runoff = numpy.empty(inputs_after_snow.shape, dtype=numpy.float32)
        ...
        evap_losses[valid_mask] = 0.0
        evap_losses[evap_mask] = numpy.minimum(
            ((absevap[evap_mask] + aint[evap_mask]) * inputs_after_runoff[evap_mask]),
            (0.4 * pet_rem[evap_mask]),
        )
        # remaining inputs after evaporation
        inputs_after_surface = numpy.empty(inputs_after_snow.shape, dtype=numpy.float32)
...
def calc_potential_transpiration(return_type):
    ...
    def _calc_potential_transpiration(
        ...
        trap[no_transpiration_mask] = 0.0

        transpiration_mask = valid_mask & (tave >= 2)
        trap[transpiration_mask] = numpy.maximum(
            numpy.minimum(
                trap[transpiration_mask],
                pet_rem[transpiration_mask]
                * 0.65
                * (1 - numpy.exp(-0.02 * aliv[transpiration_mask])),
            ),
            0.0,
        )
        ...
        pevp[valid_mask] = numpy.maximum(
            pet_rem[valid_mask] - trap[valid_mask] - evap_losses[valid_mask], 0.0
        )
        ...
        modified_moisture_inputs = numpy.empty(pet_rem.shape, dtype=numpy.float32)
...
def calc_available_water_for_transpiration(asmos, awilt, adep):
    ...
    """
    valid_mask = (
        (asmos != _TARGET_NODATA) & (awilt != _TARGET_NODATA) & (adep != _IC_NODATA)
    )
    avw = numpy.empty(asmos.shape, dtype=numpy.float32)
    avw[:] = _TARGET_NODATA
    avw[valid_mask] = numpy.maximum(
        asmos[valid_mask] - awilt[valid_mask] * adep[valid_mask], 0.0
    )
    return avw
...
def revise_potential_transpiration(trap, tot):
    ...
    """
    valid_mask = (trap != _TARGET_NODATA) & (tot != _TARGET_NODATA)
...
def remove_transpiration(return_type):
    ...
    def _remove_transpiration(asmos, awilt, adep, trap, awwt, tot2):
        ...
        transpiration_loss[transpire_mask] = numpy.minimum(
            (trap[transpire_mask] * awwt[transpire_mask]) / tot2[transpire_mask],
        ...
        asmos_revised[:] = _TARGET_NODATA
        asmos_revised[valid_mask] = asmos[valid_mask] - transpiration_loss[valid_mask]
...
def calc_evaporation_loss(rwcf_1, pevp, absevap, asmos_1, awilt_1, adep_1):
    ...
    evmt[:] = _TARGET_NODATA
    evmt[valid_mask] = numpy.maximum((rwcf_1[valid_mask] - 0.25) / (1 - 0.25), 0.01)
    ...
    evlos[valid_mask] = numpy.minimum(
        ...
        numpy.maximum(
            asmos_1[valid_mask] - awilt_1[valid_mask] * adep_1[valid_mask], 0.0
        ),
    )
    return evlos
...
def _soil_water(
    ...
    for val in ["avw", "awwt", "avinj"]:
        for lyr in range(1, nlaypg_max + 1):
            ...
            temp_val_dict[val_lyr] = os.path.join(temp_dir, "{}.tif".format(val_lyr))
    ...
    pygeoprocessing.raster_calculator(
        [
            (path, 1)
            for path in [prev_sv_reg["strucc_1_path"], prev_sv_reg["metabc_1_path"]]
        ],
        calc_surface_litter_biomass,
        temp_val_dict["alit"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    ...
    pygeoprocessing.raster_calculator(
        [
            (path, 1)
            for path in [temp_val_dict["sum_aglivc"], temp_val_dict["sum_tgprod"]]
        ],
        _calc_aboveground_live_biomass,
        temp_val_dict["aliv"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    ...
    pygeoprocessing.raster_calculator(
        [(path, 1) for path in [temp_val_dict["aliv"], temp_val_dict["sum_stdedc"]]],
        _calc_standing_biomass,
        temp_val_dict["sd"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    ...
    _snow(
        ...
        aligned_inputs["max_temp_{}".format(current_month)],
        aligned_inputs["min_temp_{}".format(current_month)],
        prev_sv_reg["snow_path"],
        prev_sv_reg["snlq_path"],
        current_month,
        month_reg["snowmelt"],
        sv_reg["snow_path"],
        sv_reg["snlq_path"],
    ...
    pygeoprocessing.raster_calculator(
        [
            ...
            for path in [
                temp_val_dict["current_moisture_inputs"],
                param_val_dict["fracro"],
                param_val_dict["precro"],
                sv_reg["snow_path"],
                temp_val_dict["alit"],
                temp_val_dict["sd"],
                param_val_dict["fwloss_1"],
                param_val_dict["fwloss_2"],
                temp_val_dict["pet_rem"],
            ]
        ],
    ...
    pygeoprocessing.raster_calculator(
        ...
        calc_potential_transpiration("trap"),
        temp_val_dict["trap"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    ...
    pygeoprocessing.raster_calculator(
        ...
        calc_potential_transpiration("pevp"),
        temp_val_dict["pevp"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    ...
    for lyr in range(1, nlayer_max + 1):
        ...
        shutil.copyfile(
            temp_val_dict["modified_moisture_inputs"], month_reg["amov_{}".format(lyr)]
        )
    ...
    raster_list_sum(avw_list, _TARGET_NODATA, temp_val_dict["tot"], _TARGET_NODATA)
    ...
    for lyr in range(1, nlaypg_max + 1):
        pygeoprocessing.raster_calculator(
            ...
            temp_val_dict["avinj_{}".format(lyr)],
            gdal.GDT_Float32,
            _TARGET_NODATA,
        )
        pygeoprocessing.raster_calculator(
    ...
    pygeoprocessing.raster_calculator(
        [
            ...
            for path in [
                sv_reg["asmos_1_path"],
                param_val_dict["adep_1"],
                pp_reg["awilt_1_path"],
                pp_reg["afiel_1_path"],
            ]
        ],
        calc_relative_water_content_lyr_1,
        temp_val_dict["rwcf_1"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    ...
    raster_difference(
        temp_val_dict["avinj_interim_1"],
        _TARGET_NODATA,
        temp_val_dict["evlos"],
        _TARGET_NODATA,
        temp_val_dict["avinj_1"],
        _TARGET_NODATA,
    )
    ...
    for pft_i in pft_id_set:
        ...
        soil_layers_accessible = [
            temp_val_dict["avinj_{}".format(lyr)]
            for lyr in range(1, int(veg_trait_table[pft_i]["nlaypg"]) + 1)
        ]
        raster_list_sum(
            soil_layers_accessible,
            _TARGET_NODATA,
    ...
    soil_layers_to_sum = [temp_val_dict["avinj_{}".format(lyr)] for lyr in [1, 2]]
    raster_list_sum(
        soil_layers_to_sum,
        _TARGET_NODATA,
        sv_reg["avh2o_3_path"],
...
def esched(return_type):
    ...
    def _esched(cflow, tca, rcetob, anps, labile):
        ...
        outofa[:] = _IC_NODATA
        outofa[valid_mask] = anps[valid_mask] * (cflow[valid_mask] / tca[valid_mask])

        immobil_ratio = numpy.zeros(cflow.shape)
        nonzero_mask = (outofa > 0) & valid_mask
        immobil_ratio[nonzero_mask] = cflow[nonzero_mask] / outofa[nonzero_mask]

        immflo = numpy.zeros(cflow.shape)
        immflo[valid_mask] = cflow[valid_mask] / rcetob[valid_mask] - outofa[valid_mask]
        ...
        # immobilization
        immobilization_mask = (
            (immobil_ratio > rcetob) & (labile_supply > 0) & valid_mask
        )
        # mineralization
        mineralization_mask = (immobil_ratio <= rcetob) & valid_mask
        # no movement
        no_movt_mask = (immobil_ratio > rcetob) & (labile_supply <= 0) & valid_mask
        ...
        material_leaving_a[immobilization_mask] = outofa[immobilization_mask]
        ...
        material_leaving_a[no_movt_mask] = 0.0
        material_arriving_b[no_movt_mask] = 0.0
        mnrflo[no_movt_mask] = 0.0
...
def fsfunc(minerl_1_2, sorpmx, pslsrb):
    ...
    c_ar[valid_mask] = sorpmx[valid_mask] * (2.0 - pslsrb[valid_mask]) / 2.0
    b_ar = numpy.zeros(minerl_1_2.shape, dtype=numpy.float32)
    b_ar[valid_mask] = sorpmx[valid_mask] - minerl_1_2[valid_mask] + c_ar[valid_mask]

    sq_ar = numpy.zeros(minerl_1_2.shape, dtype=numpy.float32)
    sq_ar[valid_mask] = (
        b_ar[valid_mask] * b_ar[valid_mask]
        + 4.0 * c_ar[valid_mask] * minerl_1_2[valid_mask]
    )
    ...
    labile[valid_mask] = (-b_ar[valid_mask] + sqrt_ar[valid_mask]) / 2.0
...
def calc_surface_som2_ratio(
    som1c_1, som1e_1_iel, rad1p_1_iel, rad1p_2_iel, rad1p_3_iel, pcemic1_2_iel
):
    ...
    radds1[:] = _TARGET_NODATA
    radds1[valid_mask] = rad1p_1_iel[valid_mask] + rad1p_2_iel[valid_mask] * (
        (som1c_1[valid_mask] / som1e_1_iel[valid_mask]) - pcemic1_2_iel[valid_mask]
    )
...
def calc_tcflow_strucc_1(
    ...
    potential_flow[valid_mask] = (
        ...
        * defac[valid_mask]
        * dec1_1[valid_mask]
        * numpy.exp(-pligst_1[valid_mask] * strlig_1[valid_mask])
        * 0.020833
    ...
    tcflow_strucc_1[valid_mask] = 0.0
...
def calc_tcflow_strucc_2(
    ...
    potential_flow[valid_mask] = (
        ...
        * defac[valid_mask]
        * dec1_2[valid_mask]
        * numpy.exp(-pligst_2[valid_mask] * strlig_2[valid_mask])
        * 0.020833
        * pheff_struc[valid_mask]
        * anerb[valid_mask]
    )
    ...
    tcflow_strucc_2[valid_mask] = 0.0
...
def calc_tcflow_surface(
    ...
    potential_flow = numpy.zeros(aminrl_1.shape, dtype=numpy.float32)
    potential_flow[valid_mask] = numpy.minimum(
        cstatv[valid_mask]
        * defac[valid_mask]
        * dec_param[valid_mask]
        * 0.020833
        * pheff[valid_mask],
        cstatv[valid_mask],
    )
...
def calc_som3_flow(tcflow, fps, animpt, anerb):
    ...
    tosom3[valid_mask] = (
        tcflow[valid_mask]
        * fps[valid_mask]
        * (1.0 + animpt[valid_mask] * (1.0 - anerb[valid_mask]))
    )
    return tosom3
...
def calc_som2_flow(som2c_1, cmix, defac):
    ...
    tcflow[:] = _IC_NODATA
    tcflow[valid_mask] = (
        som2c_1[valid_mask] * cmix[valid_mask] * defac[valid_mask] * 0.020833
    )
    return tcflow
...
def update_gross_mineralization(gross_mineralization, mineral_flow):
    ...
    """
    valid_mask = (gross_mineralization != _TARGET_NODATA) & (mineral_flow != _IC_NODATA)

    gromin_updated = numpy.empty(gross_mineralization.shape, dtype=numpy.float32)
...
def calc_net_cflow(cflow, frac_co2):
    ...
    """
    valid_mask = (cflow != _IC_NODATA) & (frac_co2 != _IC_NODATA)
...
def calc_net_cflow_tosom2(tcflow, frac_co2, tosom3, cleach):
    ...
    net_tosom2[valid_mask] = (
        tcflow[valid_mask]
        - (tcflow[valid_mask] * frac_co2[valid_mask])
        - tosom3[valid_mask]
        - cleach[valid_mask]
    )
    return net_tosom2
...
def calc_net_cflow_tosom1(tcflow, frac_co2, tosom3):
    ...
    """
    valid_mask = (
        (tcflow != _IC_NODATA) & (frac_co2 != _IC_NODATA) & (tosom3 != _IC_NODATA)
    )
...
def respiration(
    ...
    with tempfile.NamedTemporaryFile(
        ...
        operand_temp_path = operand_temp_file.name
    with tempfile.NamedTemporaryFile(
        prefix="d_statv_temp", dir=PROCESSING_DIR
    ) as d_statv_temp_file:
        d_statv_temp_path = d_statv_temp_file.name

    pygeoprocessing.raster_calculator(
        [(path, 1) for path in [tcflow_path, frac_co2_path, estatv_path, cstatv_path]],
    ...
    if gromin_1_path:
        ...
        pygeoprocessing.raster_calculator(
            [(path, 1) for path in [d_statv_temp_path, operand_temp_path]],
            update_gross_mineralization,
            gromin_1_path,
            gdal.GDT_Float32,
            _TARGET_NODATA,
        )
...
def nutrient_flow(
    cflow_path,
    cstatv_donating_path,
    estatv_donating_path,
    rcetob_path,
    minerl_1_path,
    d_estatv_donating_path,
    d_estatv_receiving_path,
    d_minerl_path,
    gromin_path=None,
):
...
def calc_c_leach(amov_2, tcflow, omlech_3, orglch):
    ...
    linten[valid_mask] = numpy.minimum(
        (1.0 - (omlech_3[valid_mask] - amov_2[valid_mask]) / omlech_3[valid_mask]), 1.0
    )

    leach_mask = (amov_2 > 0) & valid_mask
    cleach[leach_mask] = tcflow[leach_mask] * orglch[leach_mask] * linten[leach_mask]
    return cleach


def remove_leached_iel(
    som1c_2_path, som1e_2_iel_path, cleach_path, d_som1e_2_iel_path, iel
):
    ...
    def calc_leached_N(som1c_2, som1e_2_1, cleach):
        ...
        rceof1_1[valid_mask] = som1c_2[valid_mask] / som1e_2_1[valid_mask] * 2.0
    ...
    def calc_leached_P(som1c_2, som1e_2_2, cleach):
        ...
        rceof1_2[valid_mask] = som1c_2[valid_mask] / som1e_2_2[valid_mask] * 35.0
        orgflow = numpy.empty(som1c_2.shape, dtype=numpy.float32)
        orgflow[:] = _IC_NODATA
    ...
    if iel == 1:
        pygeoprocessing.raster_calculator(
            [(path, 1) for path in [som1c_2_path, som1e_2_iel_path, cleach_path]],
            calc_leached_N,
            operand_temp_path,
            gdal.GDT_Float32,
            _TARGET_NODATA,
...
def calc_pflow(pstatv, rate_param, defac):
    ...
    pflow[:] = _IC_NODATA
    pflow[valid_mask] = (
        pstatv[valid_mask] * rate_param[valid_mask] * defac[valid_mask] * 0.020833
    )
    return pflow
...
def update_aminrl(
    minerl_1_1_path, minerl_1_2_path, fsol_path, aminrl_1_path, aminrl_2_path
):
    ...
    def update_aminrl_1(aminrl_1_prev, minerl_1_1):
        ...
        valid_mask = (~numpy.isclose(aminrl_1_prev, _SV_NODATA)) & (
            ~numpy.isclose(minerl_1_1, _SV_NODATA)
        )
        aminrl_1 = numpy.empty(aminrl_1_prev.shape, dtype=numpy.float32)
        aminrl_1[:] = _SV_NODATA
        aminrl_1[valid_mask] = aminrl_1_prev[valid_mask] + minerl_1_1[valid_mask] / 2.0
        return aminrl_1
    ...
    def update_aminrl_2(aminrl_2_prev, minerl_1_2, fsol):
        ...
        aminrl_2[valid_mask] = (
            ...
            + (minerl_1_2[valid_mask] * fsol[valid_mask]) / 2.0
        )
        return aminrl_2

    with tempfile.NamedTemporaryFile(
        prefix="aminrl_prev", dir=PROCESSING_DIR
    ) as aminrl_prev_file:
    ...
    pygeoprocessing.raster_calculator(
        [(path, 1) for path in [aminrl_prev_path, minerl_1_2_path, fsol_path]],
        update_aminrl_2,
        aminrl_2_path,
        gdal.GDT_Float32,
        _SV_NODATA,
...
def sum_biomass(weighted_live_c, weighted_dead_c, strucc_1, metabc_1, elitst):
    ...
    biomass[valid_mask] = (
        weighted_live_c[valid_mask] + weighted_dead_c[valid_mask]
    ) * 2.5 + (strucc_1[valid_mask] + metabc_1[valid_mask]) * 2.5 * elitst[valid_mask]
    return biomass
...
def _decomposition(
    ...
    def calc_N_fixation(precip, annual_precip, baseNdep, epnfs_2):
        ...
        wdfxm[valid_mask] = baseNdep[valid_mask] * (
            precip[valid_mask] / annual_precip[valid_mask]
        ) + epnfs_2[valid_mask] * numpy.minimum(annual_precip[valid_mask], 100.0) * (
            precip[valid_mask] / annual_precip[valid_mask]
        )
        return wdfxm
    ...
    def calc_rprpet(pevap, snowmelt, avh2o_3, precip):
        ...
        rprpet[no_melt_mask] = (avh2o_3[no_melt_mask] + precip[no_melt_mask]) / pevap[
            no_melt_mask
        ]
        return rprpet
    ...
    def calc_bgwfunc(rprpet):
        ...
        bgwfunc[valid_mask] = 1.0 / (1.0 + 30 * numpy.exp(-8.5 * rprpet[valid_mask]))
        bgwfunc[(valid_mask & (rprpet > 9))] = 1
        return bgwfunc

    def calc_stemp(biomass, snow, max_temp, min_temp, daylength, pmntmp, pmxtmp):
        ...
        tmxs[valid_mask] = max_temp[valid_mask] + (
            25.4 / (1.0 + 18.0 * numpy.exp(-0.2 * max_temp[valid_mask]))
        ) * (numpy.exp(pmxtmp[valid_mask] * biomass[valid_mask]) - 0.13)
        tmns = numpy.empty(biomass.shape, dtype=numpy.float32)
        tmns[valid_mask] = (
            min_temp[valid_mask] + pmntmp[valid_mask] * biomass[valid_mask] - 1.78
        )

        shortday_mask = (daylength < 12.0) & valid_mask
        snow_mask = (snow > 0) & valid_mask

        tmns_mlt = numpy.empty(biomass.shape, dtype=numpy.float32)
        tmns_mlt[valid_mask] = ((12.0 - daylength[valid_mask]) * 1.2 + 12.0) / 24.0
        tmns_mlt[shortday_mask] = ((12 - daylength[shortday_mask]) * 3.0 + 12.0) / 24.0
        ...
        stemp[valid_mask] = (1 - tmns_mlt[valid_mask]) * tmxs[valid_mask] + tmns_mlt[
            valid_mask
        ] * tmns[valid_mask]
        stemp[snow_mask] = 0.0
        return stemp
    ...
    def calc_defac(bgwfunc, stemp, teff_1, teff_2, teff_3, teff_4):
        ...
        tfunc[valid_mask] = numpy.maximum(
            ...
            / (
                ...
                + (teff_3[valid_mask] / numpy.pi)
                * numpy.arctan(
                    numpy.pi * teff_4[valid_mask] * (30.0 - teff_1[valid_mask])
                )
            ),
        )
        ...
        defac[:] = _TARGET_NODATA
        defac[valid_mask] = numpy.maximum(0.0, tfunc[valid_mask] * bgwfunc[valid_mask])
        return defac
    ...
    def calc_pheff_struc(pH):
        ...
        pheff_struc[valid_mask] = numpy.clip(
            (
                0.5
                + (1.1 / numpy.pi)
                * numpy.arctan(numpy.pi * 0.7 * (pH[valid_mask] - 4.0))
            ),
            0,
            1,
        )
    ...
    def calc_pheff_metab(pH):
        ...
        pheff_metab[valid_mask] = numpy.clip(
            (
                0.5
                + (1.14 / numpy.pi)
                * numpy.arctan(numpy.pi * 0.7 * (pH[valid_mask] - 4.8))
            ),
            0,
            1,
        )
    ...
    def calc_pheff_som3(pH):
        ...
        pheff_metab[valid_mask] = numpy.clip(
            (
                0.5
                + (1.1 / numpy.pi)
                * numpy.arctan(numpy.pi * 0.7 * (pH[valid_mask] - 3.0))
            ),
            0,
            1,
        )
    ...
    pH_nodata = pygeoprocessing.get_raster_info(aligned_inputs["ph_path"])["nodata"][0]
    ...
    # estimated daylength
    _calc_daylength(
        aligned_inputs["site_index"], current_month, temp_val_dict["daylength"]
    )
    ...
    pygeoprocessing.raster_calculator(
        [
            ...
            for path in [
                ...
                temp_val_dict["sum_stdedc"],
                prev_sv_reg["strucc_1_path"],
                prev_sv_reg["metabc_1_path"],
                param_val_dict["elitst"],
            ]
        ],
        sum_biomass,
        temp_val_dict["biomass"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    ...
    pygeoprocessing.raster_calculator(
        [
            ...
            for path in [
                month_reg["bgwfunc"],
                temp_val_dict["stemp"],
                param_val_dict["teff_1"],
                param_val_dict["teff_2"],
                param_val_dict["teff_3"],
                param_val_dict["teff_4"],
            ]
        ],
        calc_defac,
        temp_val_dict["defac"],
    ...
    pygeoprocessing.raster_calculator(
        [
            ...
            for path in [
                temp_val_dict["rprpet"],
                temp_val_dict["pevap"],
                param_val_dict["drain"],
                param_val_dict["aneref_1"],
                param_val_dict["aneref_2"],
                param_val_dict["aneref_3"],
            ]
        ],
        calc_anerb,
        temp_val_dict["anerb"],
    ...
    for lyr in range(1, nlayer_max + 1):
        ...
        delta_sv_dict[state_var] = os.path.join(temp_dir, "{}.tif".format(state_var))
    ...
    for dtm in range(4):
        ...
        for lyr in [1, 2]:
                ...
                pygeoprocessing.raster_calculator(
                    [
                        ...
                        for path in [
                            ...
                            pp_reg["rnewas_1_1_path"],
                            pp_reg["rnewas_2_1_path"],
                            param_val_dict["strmax_1"],
                            temp_val_dict["defac"],
                            param_val_dict["dec1_1"],
                            param_val_dict["pligst_1"],
                            sv_reg["strlig_1_path"],
                            temp_val_dict["pheff_struc"],
                        ]
                    ],
                    calc_tcflow_strucc_1,
                ...
                pygeoprocessing.raster_calculator(
                    [
                        ...
                        for path in [
                            temp_val_dict["aminrl_1"],
                            temp_val_dict["aminrl_2"],
                            sv_reg["strucc_2_path"],
                            sv_reg["struce_2_1_path"],
                            sv_reg["struce_2_2_path"],
                            pp_reg["rnewbs_1_1_path"],
                            pp_reg["rnewbs_2_1_path"],
                            param_val_dict["strmax_2"],
                            temp_val_dict["defac"],
                            param_val_dict["dec1_2"],
                            param_val_dict["pligst_2"],
                            sv_reg["strlig_2_path"],
                            temp_val_dict["pheff_struc"],
                            temp_val_dict["anerb"],
                        ]
                    ],
                    calc_tcflow_strucc_2,
                    temp_val_dict["tcflow"],
                    gdal.GDT_Float32,
                    _IC_NODATA,
                )
            shutil.copyfile(
                delta_sv_dict["strucc_{}".format(lyr)], temp_val_dict["d_statv_temp"]
            )
            ...
            pygeoprocessing.raster_calculator(
                [
                    (path, 1)
                    for path in [temp_val_dict["tosom2"], param_val_dict["rsplig"]]
                ],
                calc_net_cflow,
            ...
            shutil.copyfile(
                delta_sv_dict["som2c_{}".format(lyr)], temp_val_dict["d_statv_temp"]
            )
            raster_sum(
                temp_val_dict["d_statv_temp"],
                _IC_NODATA,
            ...
            shutil.copyfile(
                delta_sv_dict["som1c_{}".format(lyr)], temp_val_dict["d_statv_temp"]
            )
            raster_sum(
                temp_val_dict["d_statv_temp"],
                _IC_NODATA,
        ...
        for lyr in [1, 2]:
                ...
                pygeoprocessing.raster_calculator(
                    [
                        ...
                        for path in [
                            ...
                            temp_val_dict["rceto1_1"],
                            temp_val_dict["rceto1_2"],
                            temp_val_dict["defac"],
                ...
                pygeoprocessing.raster_calculator(
                    [
                        ...
                        for path in [
                            temp_val_dict["aminrl_1"],
                            temp_val_dict["aminrl_2"],
                            sv_reg["metabc_2_path"],
                            sv_reg["metabe_2_1_path"],
                            sv_reg["metabe_2_2_path"],
                            temp_val_dict["rceto1_1"],
                            temp_val_dict["rceto1_2"],
                            temp_val_dict["defac"],
                            param_val_dict["dec2_2"],
                            temp_val_dict["pheff_metab"],
                            temp_val_dict["anerb"],
                        ]
                    ],
            ...
            shutil.copyfile(
                delta_sv_dict["metabc_{}".format(lyr)], temp_val_dict["d_statv_temp"]
            )
            raster_difference(
                temp_val_dict["d_statv_temp"],
                _IC_NODATA,
        ...
        pygeoprocessing.raster_calculator(
            [
                ...
                for path in [
                    ...
                    temp_val_dict["aminrl_2"],
                    sv_reg["som1c_1_path"],
                    sv_reg["som1e_1_1_path"],
                    sv_reg["som1e_1_2_path"],
                    temp_val_dict["rceto2_1"],
                    temp_val_dict["rceto2_2"],
                    temp_val_dict["defac"],
        ...
        shutil.copyfile(delta_sv_dict["som1c_1"], temp_val_dict["d_statv_temp"])
        raster_difference(
            temp_val_dict["d_statv_temp"],
            _IC_NODATA,
            temp_val_dict["tcflow"],
            _IC_NODATA,
            delta_sv_dict["som1c_1"],
            _IC_NODATA,
        )
        # microbial respiration with decomposition to SOM2
        respiration(
            temp_val_dict["tcflow"],
            param_val_dict["p1co2a_1"],
            sv_reg["som1c_1_path"],
            sv_reg["som1e_1_1_path"],
            delta_sv_dict["som1e_1_1"],
            delta_sv_dict["minerl_1_1"],
            gromin_1_path=temp_val_dict["gromin_1"],
        )
        respiration(
        ...
        pygeoprocessing.raster_calculator(
            [
                (path, 1)
                for path in [temp_val_dict["tcflow"], param_val_dict["p1co2a_1"]]
            ],
            calc_net_cflow,
            temp_val_dict["net_tosom2"],
            gdal.GDT_Float32,
        ...
        shutil.copyfile(delta_sv_dict["som2c_1"], temp_val_dict["d_statv_temp"])
        raster_sum(
            temp_val_dict["d_statv_temp"],
            _IC_NODATA,
            temp_val_dict["net_tosom2"],
            _IC_NODATA,
            delta_sv_dict["som2c_1"],
            _IC_NODATA,
        )
        ...
        pygeoprocessing.raster_calculator(
            [
                ...
                for path in [
                    ...
                    temp_val_dict["aminrl_2"],
                    sv_reg["som1c_2_path"],
                    sv_reg["som1e_2_1_path"],
                    sv_reg["som1e_2_2_path"],
                    temp_val_dict["rceto2_1"],
                    temp_val_dict["rceto2_2"],
                    temp_val_dict["defac"],
                    param_val_dict["dec3_2"],
                    pp_reg["eftext_path"],
                    temp_val_dict["anerb"],
                    temp_val_dict["pheff_metab"],
                ]
        ...
        shutil.copyfile(delta_sv_dict["som1c_2"], temp_val_dict["d_statv_temp"])
        raster_difference(
            temp_val_dict["d_statv_temp"],
            _IC_NODATA,
            temp_val_dict["tcflow"],
            _IC_NODATA,
            delta_sv_dict["som1c_2"],
            _IC_NODATA,
        )
        ...
        nutrient_flow(
            ...
            sv_reg["som1c_2_path"],
            sv_reg["som1e_2_1_path"],
            temp_val_dict["rceto3_1"],
            sv_reg["minerl_1_1_path"],
            delta_sv_dict["som1e_2_1"],
            delta_sv_dict["som3e_1"],
            delta_sv_dict["minerl_1_1"],
            gromin_path=temp_val_dict["gromin_1"],
        )
        nutrient_flow(
            ...
            sv_reg["som1c_2_path"],
            sv_reg["som1e_2_2_path"],
            temp_val_dict["rceto3_2"],
            sv_reg["minerl_1_2_path"],
            delta_sv_dict["som1e_2_2"],
            delta_sv_dict["som3e_2"],
            delta_sv_dict["minerl_1_2"],
        )
        ...
        for iel in [1, 2]:
            remove_leached_iel(
                sv_reg["som1c_2_path"],
                sv_reg["som1e_2_{}_path".format(iel)],
                temp_val_dict["cleach"],
                delta_sv_dict["som1e_2_{}".format(iel)],
                iel,
            )
        ...
        pygeoprocessing.raster_calculator(
            ...
            calc_net_cflow_tosom2,
            temp_val_dict["net_tosom2"],
            gdal.GDT_Float32,
            _IC_NODATA,
        )
        shutil.copyfile(delta_sv_dict["som2c_2"], temp_val_dict["d_statv_temp"])
        raster_sum(
            temp_val_dict["d_statv_temp"],
            _IC_NODATA,
            temp_val_dict["net_tosom2"],
            _IC_NODATA,
            delta_sv_dict["som2c_2"],
            _IC_NODATA,
        )
        ...
        pygeoprocessing.raster_calculator(
            [
                ...
                for path in [
                    ...
                    temp_val_dict["aminrl_2"],
                    sv_reg["som2c_2_path"],
                    sv_reg["som2e_2_1_path"],
                    sv_reg["som2e_2_2_path"],
                    temp_val_dict["rceto1_1"],
                    temp_val_dict["rceto1_2"],
                    temp_val_dict["defac"],
                    param_val_dict["dec5_2"],
                    temp_val_dict["pheff_metab"],
                    temp_val_dict["anerb"],
                ]
            ],
        ...
        pygeoprocessing.raster_calculator(
            [
                ...
                for path in [
                    temp_val_dict["tcflow"],
                    param_val_dict["p2co2_2"],
                    temp_val_dict["tosom3"],
                ]
            ],
            calc_net_cflow_tosom1,
            temp_val_dict["net_tosom1"],
            gdal.GDT_Float32,
            _IC_NODATA,
        )
        ...
        pygeoprocessing.raster_calculator(
            [
                ...
                for path in [
                    ...
                    temp_val_dict["aminrl_2"],
                    sv_reg["som2c_1_path"],
                    sv_reg["som2e_1_1_path"],
                    sv_reg["som2e_1_2_path"],
                    temp_val_dict["rceto1_1"],
                    temp_val_dict["rceto1_2"],
                    temp_val_dict["defac"],
                    param_val_dict["dec5_1"],
                    temp_val_dict["pheff_struc"],
                ]
            ],
            calc_tcflow_surface,
        ...
        pygeoprocessing.raster_calculator(
            [
                (path, 1)
                for path in [temp_val_dict["tcflow"], param_val_dict["p2co2_1"]]
            ],
            calc_net_cflow,
            temp_val_dict["tosom1"],
            gdal.GDT_Float32,
        ...
        pygeoprocessing.raster_calculator(
            [(path, 1) for path in [temp_val_dict["tcflow"], param_val_dict["p3co2"]]],
            calc_net_cflow,
            temp_val_dict["tosom1"],
            gdal.GDT_Float32,
            _IC_NODATA,
        ...
        pygeoprocessing.raster_calculator(
            [
                (path, 1)
                for path in [
                    sv_reg["parent_2_path"],
                    param_val_dict["pparmn_2"],
                    temp_val_dict["defac"],
                ]
            ],
            calc_pflow,
        ...
        shutil.copyfile(delta_sv_dict["parent_2"], temp_val_dict["d_statv_temp"])
        raster_difference(
            temp_val_dict["d_statv_temp"],
            _IC_NODATA,
            temp_val_dict["pflow"],
            _IC_NODATA,
            delta_sv_dict["parent_2"],
            _IC_NODATA,
        )
        shutil.copyfile(delta_sv_dict["minerl_1_2"], temp_val_dict["d_statv_temp"])
        raster_sum(
            temp_val_dict["d_statv_temp"],
            _IC_NODATA,
            temp_val_dict["pflow"],
        ...
        pygeoprocessing.raster_calculator(
            [
                (path, 1)
                for path in [
                    sv_reg["secndy_2_path"],
                    param_val_dict["psecmn_2"],
                    temp_val_dict["defac"],
                ]
            ],
            calc_pflow,
        ...
        shutil.copyfile(delta_sv_dict["secndy_2"], temp_val_dict["d_statv_temp"])
        raster_difference(
            temp_val_dict["d_statv_temp"],
            _IC_NODATA,
            temp_val_dict["pflow"],
            _IC_NODATA,
            delta_sv_dict["secndy_2"],
            _IC_NODATA,
        )
        shutil.copyfile(delta_sv_dict["minerl_1_2"], temp_val_dict["d_statv_temp"])
        raster_sum(
            ...
            pygeoprocessing.raster_calculator(
                [
                    ...
                    for path in [
                        ...
                        param_val_dict["pmnsec_2"],
                        temp_val_dict["fsol"],
                        temp_val_dict["defac"],
                    ]
                ],
                calc_pflow_to_secndy,
                temp_val_dict["pflow"],
                gdal.GDT_Float64,
                _IC_NODATA,
            )
            shutil.copyfile(
                delta_sv_dict["minerl_{}_2".format(lyr)], temp_val_dict["d_statv_temp"]
            )
            raster_difference(
                temp_val_dict["d_statv_temp"],
                _IC_NODATA,
        ...
        shutil.copyfile(delta_sv_dict["occlud"], temp_val_dict["d_statv_temp"])
        raster_sum(
            temp_val_dict["d_statv_temp"],
            _IC_NODATA,
            temp_val_dict["pflow"],
            _IC_NODATA,
            delta_sv_dict["occlud"],
            _IC_NODATA,
        )
        ...
        state_var = "{}c".format(compartment)
        shutil.copyfile(
            sv_reg["{}_path".format(state_var)], temp_val_dict["operand_temp"]
        )
        raster_sum(
            delta_sv_dict[state_var],
            _IC_NODATA,
    ...
    shutil.copyfile(sv_reg["minerl_1_1_path"], temp_val_dict["d_statv_temp"])
    raster_difference(
        temp_val_dict["d_statv_temp"],
        _SV_NODATA,
        temp_val_dict["operand_temp"],
...
def partit(
    ...
    def calc_dirabs(cpart, epart_iel, minerl_1_iel, damr_lyr_iel, pabres, damrmn_iel):
        ...
        dirabs_iel[valid_mask] = 0.0

        minerl_mask = (minerl_1_iel >= 0) & valid_mask
        dirabs_iel[minerl_mask] = (
            damr_lyr_iel[minerl_mask]
            * minerl_1_iel[minerl_mask]
            * numpy.maximum(cpart[minerl_mask] / pabres[minerl_mask], 1.0)
        )
        ...
        rcetot[valid_mask] = 0
        rcetot[e_sufficient_mask] = cpart[e_sufficient_mask] / (
    ...
    def calc_d_metabc_lyr(cpart, epart_1, dirabs_1, frlign, spl_1, spl_2):
        ...
        rlnres[valid_mask] = 0.0
        rlnres[movt_mask] = frlign[movt_mask] / (
            (epart_1[movt_mask] + dirabs_1[movt_mask]) / (cpart[movt_mask] * 2.5)
        )

        # frmet: fraction of cpart that goes to metabolic
        frmet = numpy.empty(cpart.shape, dtype=numpy.float32)
        frmet[:] = _TARGET_NODATA
        frmet[valid_mask] = spl_1[valid_mask] - spl_2[valid_mask] * rlnres[valid_mask]

        lign_exceeded_mask = (frlign > (1.0 - frmet)) & valid_mask
        frmet[lign_exceeded_mask] = 1.0 - frlign[lign_exceeded_mask]
    ...
    def calc_d_strucc_lyr(cpart, d_metabc_lyr):
        ...
        """
        valid_mask = (cpart != _TARGET_NODATA) & (d_metabc_lyr != _TARGET_NODATA)
    ...
    def calc_d_struce_lyr_iel(d_strucc_lyr, rcestr_iel):
        ...
        """
        valid_mask = (d_strucc_lyr != _TARGET_NODATA) & (rcestr_iel != _IC_NODATA)

        d_struce_lyr_iel = numpy.empty(d_strucc_lyr.shape, dtype=numpy.float32)
        d_struce_lyr_iel[valid_mask] = d_strucc_lyr[valid_mask] / rcestr_iel[valid_mask]
    ...
    def calc_d_metabe_lyr_iel(cpart, epart_iel, dirabs_iel, d_struce_lyr_iel):
        ...
        d_metabe_lyr_iel[valid_mask] = (
            epart_iel[valid_mask]
            + dirabs_iel[valid_mask]
    ...
    def calc_d_strlig_lyr(frlign, d_strucc_lyr, cpart, strlig_lyr, strucc_lyr):
        ...
        fligst[valid_mask] = 1.0
        fligst[movt_mask] = numpy.minimum(
            frlign[movt_mask] / (d_strucc_lyr[movt_mask] / cpart[movt_mask]), 1.0
        )
        ...
        d_strlig_lyr[valid_mask] = strlig_lyr_mod[valid_mask] - strlig_lyr[valid_mask]
        return d_strlig_lyr
    ...
    for iel in [1, 2]:
        ...
        shutil.copyfile(
            sv_reg["minerl_1_{}_path".format(iel)], temp_val_dict["d_statv_temp"]
        )
        raster_difference(
            temp_val_dict["d_statv_temp"],
            _SV_NODATA,
    ...
    pygeoprocessing.raster_calculator(
        [
            ...
            for path in [
                cpart_path,
                epart_1_path,
                temp_val_dict["dirabs_1"],
                frlign_path,
                param_val_dict["spl_1"],
                param_val_dict["spl_2"],
            ]
        ],
        calc_d_metabc_lyr,
        temp_val_dict["d_metabc_lyr"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    pygeoprocessing.raster_calculator(
        [(path, 1) for path in [cpart_path, temp_val_dict["d_metabc_lyr"]]],
        calc_d_strucc_lyr,
    ...
    shutil.copyfile(sv_reg["metabc_{}_path".format(lyr)], temp_val_dict["d_statv_temp"])
    raster_sum(
        temp_val_dict["d_statv_temp"],
        _SV_NODATA,
        temp_val_dict["d_metabc_lyr"],
    ...
    shutil.copyfile(sv_reg["strucc_{}_path".format(lyr)], temp_val_dict["d_statv_temp"])
    raster_sum(
        temp_val_dict["d_statv_temp"],
        _SV_NODATA,
        temp_val_dict["d_strucc_lyr"],
    ...
    for iel in [1, 2]:
        ...
        pygeoprocessing.raster_calculator(
            ...
            calc_d_struce_lyr_iel,
            temp_val_dict["d_struce_lyr_iel"],
            gdal.GDT_Float32,
            _TARGET_NODATA,
        )
        shutil.copyfile(
            sv_reg["struce_{}_{}_path".format(lyr, iel)], temp_val_dict["d_statv_temp"]
        )
        raster_sum(
            temp_val_dict["d_statv_temp"],
            _SV_NODATA,
        ...
        pygeoprocessing.raster_calculator(
            ...
            calc_d_metabe_lyr_iel,
            temp_val_dict["operand_temp"],
            gdal.GDT_Float32,
            _TARGET_NODATA,
        )
        shutil.copyfile(
            sv_reg["metabe_{}_{}_path".format(lyr, iel)], temp_val_dict["d_statv_temp"]
        )
        raster_sum(
            temp_val_dict["d_statv_temp"],
            _SV_NODATA,
    ...
    shutil.copyfile(sv_reg["strlig_{}_path".format(lyr)], temp_val_dict["d_statv_temp"])
    raster_sum(
        temp_val_dict["d_statv_temp"],
        _SV_NODATA,
        temp_val_dict["operand_temp"],
...
def calc_fall_standing_dead(stdedc, fallrt):
    ...
    """
    valid_mask = (~numpy.isclose(stdedc, _SV_NODATA)) & (fallrt != _IC_NODATA)
...
def calc_root_death(average_temperature, rtdtmp, rdr, avh2o_1, deck5, bglivc):
    ...
    root_death_rate[valid_mask] = 0.0
    ...
    delta_c_root_death[valid_mask] = root_death_rate[valid_mask] * bglivc[valid_mask]
...
def _death_and_partition(
    ...
    for val in [
        ...
        "delta_sv_weighted",
        "operand_temp",
        "sum_weighted_delta_C",
        "sum_weighted_delta_N",
        "sum_weighted_delta_P",
        "weighted_lignin",
        "sum_lignin",
        "fraction_lignin",
    ]:
    ...
    for pft_i in pft_id_set:
        ...
        raster_difference(
            ...
            temp_val_dict["delta_c"],
            _TARGET_NODATA,
            sv_reg["{}c_{}_path".format(state_variable, pft_i)],
            _SV_NODATA,
        )
        ...
        raster_multiplication(
            ...
            aligned_inputs["pft_{}".format(pft_i)],
            pft_nodata,
            temp_val_dict["delta_sv_weighted"],
            _TARGET_NODATA,
        )
        shutil.copyfile(
            temp_val_dict["sum_weighted_delta_C"], temp_val_dict["operand_temp"]
        )
        raster_sum(
            temp_val_dict["delta_sv_weighted"],
            _TARGET_NODATA,
        ...
        raster_multiplication(
            ...
            frlign_path,
            _TARGET_NODATA,
            temp_val_dict["weighted_lignin"],
            _TARGET_NODATA,
        )
        shutil.copyfile(temp_val_dict["sum_lignin"], temp_val_dict["operand_temp"])
        raster_sum(
            temp_val_dict["weighted_lignin"],
            _TARGET_NODATA,
            temp_val_dict["operand_temp"],
        ...
        for iel in [1, 2]:
            ...
            pygeoprocessing.raster_calculator(
                [
                    ...
                    for path in [
                        prev_sv_reg["{}c_{}_path".format(state_variable, pft_i)],
                        prev_sv_reg[
                            "{}e_{}_{}_path".format(state_variable, iel, pft_i)
                        ],
                        temp_val_dict["delta_c"],
                    ]
                ],
                calc_delta_iel,
                temp_val_dict["delta_iel"],
                gdal.GDT_Float32,
                _TARGET_NODATA,
            )
            # subtract delta_iel from the pft-level state variable
            raster_difference(
                prev_sv_reg["{}e_{}_{}_path".format(state_variable, iel, pft_i)],
                _SV_NODATA,
                temp_val_dict["delta_iel"],
                _TARGET_NODATA,
            ...
            if iel == 1:
                shutil.copyfile(
                    temp_val_dict["sum_weighted_delta_N"], temp_val_dict["operand_temp"]
                )
                raster_sum(
                    temp_val_dict["delta_sv_weighted"],
                    _TARGET_NODATA,
                ...
                shutil.copyfile(
                    temp_val_dict["sum_weighted_delta_P"], temp_val_dict["operand_temp"]
                )
                raster_sum(
                    temp_val_dict["delta_sv_weighted"],
                    _TARGET_NODATA,
...
def calc_senescence_water_shading(aglivc, bgwfunc, fsdeth_1, fsdeth_3, fsdeth_4):
    ...
    fdeth[valid_mask] = fsdeth_1[valid_mask] * (1.0 - bgwfunc[valid_mask])

    shading_mask = (aglivc > fsdeth_4) & valid_mask
    fdeth[shading_mask] = fdeth[shading_mask] + fsdeth_3[shading_mask]
    fdeth[valid_mask] = numpy.minimum(fdeth[valid_mask], 1.0)
    return fdeth
...
def _shoot_senescence(
    ...
    for pft_i in pft_id_set:
        if current_month == veg_trait_table[pft_i]["senescence_month"]:
            temp_val_dict["fdeth"] = param_val_dict["fsdeth_2_{}".format(pft_i)]
        else:
            pygeoprocessing.raster_calculator(
                [
                    (path, 1)
        ...
        shutil.copyfile(
            sv_reg["stdedc_{}_path".format(pft_i)], temp_val_dict["operand_temp"]
        )
        raster_sum(
            temp_val_dict["operand_temp"],
            _SV_NODATA,
        ...
        for iel in [1, 2]:
            ...
            if iel == 1:
                ...
                raster_multiplication(
                    temp_val_dict["delta_iel"],
                    _TARGET_NODATA,
                    param_val_dict["vlossp_{}".format(pft_i)],
                    _IC_NODATA,
                    temp_val_dict["vol_loss"],
                    _TARGET_NODATA,
                )
                shutil.copyfile(
            ...
            raster_multiplication(
                temp_val_dict["delta_iel"],
                _TARGET_NODATA,
                param_val_dict["crprtf_{}_{}".format(iel, pft_i)],
                _IC_NODATA,
                temp_val_dict["to_storage"],
                _TARGET_NODATA,
            )
            raster_sum(
...
def convert_biomass_to_C(biomass_path, c_path):
    ...
    pygeoprocessing.raster_calculator(
        [(biomass_path, 1)], convert_op, c_path, gdal.GDT_Float32, _TARGET_NODATA
    )
...
def restrict_potential_growth(potenc, availm_1, availm_2, snfxmx_1):
    ...
    growth_mask = ((availm_1 > 0) | (snfxmx_1 > 0)) & (availm_2 > 0) & valid_mask
...
def c_uptake_aboveground(cprodl, rtsh):
    ...
    """
    valid_mask = (cprodl != _TARGET_NODATA) & (rtsh != _TARGET_NODATA)

    delta_aglivc = numpy.empty(cprodl.shape, dtype=numpy.float32)
    delta_aglivc[:] = _TARGET_NODATA
    delta_aglivc[valid_mask] = cprodl[valid_mask] * (
        1.0 - rtsh[valid_mask] / (rtsh[valid_mask] + 1.0)
    )
    return delta_aglivc
...
def c_uptake_belowground(bglivc, cprodl, rtsh):
    ...
    c_prod_belowground[valid_mask] = cprodl[valid_mask] * (
        rtsh[valid_mask] / (rtsh[valid_mask] + 1.0)
    )

    modified_bglivc = numpy.empty(bglivc.shape, dtype=numpy.float32)
    modified_bglivc[:] = _SV_NODATA
    modified_bglivc[valid_mask] = bglivc[valid_mask] + c_prod_belowground[valid_mask]
...
def calc_uptake_source(return_type):
    ...
    def _uptake(eavail_iel, eup_above_iel, eup_below_iel, plantNfix, storage_iel, iel):
        ...
        eprodl_iel[valid_mask] = eup_above_iel[valid_mask] + eup_below_iel[valid_mask]
        ...
        uptake_storage[valid_mask] = 0.0
        uptake_storage[storage_sufficient_mask] = eprodl_iel[storage_sufficient_mask]
        uptake_soil[storage_sufficient_mask] = 0.0
        uptake_Nfix[storage_sufficient_mask] = 0.0
        ...
        if iel == 1:
            uptake_soil[insuff_mask] = numpy.minimum(
                (
                    eprodl_iel[insuff_mask]
                    - storage_iel[insuff_mask]
                    - plantNfix[insuff_mask]
                ),
                (
                    eavail_iel[insuff_mask]
                    - storage_iel[insuff_mask]
                    - plantNfix[insuff_mask]
                ),
            )
...
def calc_aboveground_uptake(total_uptake, eup_above_iel, eup_below_iel):
    ...
    uptake_above[valid_mask] = 0.0
    uptake_above[nonzero_mask] = total_uptake[nonzero_mask] * (
...
def calc_belowground_uptake(total_uptake, eup_above_iel, eup_below_iel):
    ...
    uptake_below[valid_mask] = 0.0
    uptake_below[nonzero_mask] = total_uptake[nonzero_mask] * (
...
def calc_minerl_uptake_lyr(uptake_soil, minerl_lyr_iel, fsol, availm):
    ...
    minerl_uptake_lyr[valid_mask] = (
        uptake_soil[valid_mask]
        * minerl_lyr_iel[valid_mask]
        * fsol[valid_mask]
        / availm[valid_mask]
    )
...
def nutrient_uptake(
    ...
    pft_nodata = pygeoprocessing.get_raster_info(fract_cover_path)["nodata"][0]
    pygeoprocessing.raster_calculator(
        [
            (path, 1)
            for path in [
        ...
        + [(iel, "raw")],
        calc_uptake_source("uptake_storage"),
        temp_val_dict["uptake_storage"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    ...
    pygeoprocessing.raster_calculator(
        ...
        + [(iel, "raw")],
        calc_uptake_source("uptake_soil"),
    ...
    if iel == 1:
        ...
        pygeoprocessing.raster_calculator(
            ...
            + [(iel, "raw")],
            calc_uptake_source("uptake_Nfix"),
            temp_val_dict["uptake_Nfix"],
            gdal.GDT_Float32,
            _TARGET_NODATA,
        )
    ...
    shutil.copyfile(
        sv_reg["crpstg_{}_{}_path".format(iel, pft_i)], temp_val_dict["statv_temp"]
    )
    raster_difference(
    ...
    pygeoprocessing.raster_calculator(
        ...
        calc_belowground_uptake,
        temp_val_dict["uptake_below"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    shutil.copyfile(
        sv_reg["bglive_{}_{}_path".format(iel, pft_i)], temp_val_dict["statv_temp"]
    )
    raster_sum(
        temp_val_dict["statv_temp"],
        _SV_NODATA,
    ...
    for lyr in range(1, nlay + 1):
        if iel == 2:
            pygeoprocessing.raster_calculator(
                [
                    (path, 1)
                    for path in [sv_reg["minerl_1_2_path"], sorpmx_path, pslsrb_path]
                ],
                fsfunc,
                temp_val_dict["fsol"],
                gdal.GDT_Float32,
        ...
        pygeoprocessing.raster_calculator(
            ...
            calc_minerl_uptake_lyr,
            temp_val_dict["minerl_uptake_lyr"],
            gdal.GDT_Float32,
            _TARGET_NODATA,
        )
        ...
        raster_multiplication(
            ...
            temp_val_dict["minerl_uptake_lyr"],
            _TARGET_NODATA,
            temp_val_dict["uptake_weighted"],
            _TARGET_NODATA,
        )
        shutil.copyfile(
            sv_reg["minerl_{}_{}_path".format(lyr, iel)], temp_val_dict["statv_temp"]
        )
        raster_difference(
            temp_val_dict["statv_temp"],
            _SV_NODATA,
...
def calc_nutrient_limitation(return_type):
    ...
    def _nutrlm(
        ...
        cfrac_below[valid_mask] = rtsh[valid_mask] / (rtsh[valid_mask] + 1.0)
        cfrac_above = numpy.empty(potenc.shape, dtype=numpy.float32)
        cfrac_above[valid_mask] = 1.0 - cfrac_below[valid_mask]
        ...
        maxeci_above_1[valid_mask] = 1.0 / cercrp_min_above_1[valid_mask]
        mineci_above_1[valid_mask] = 1.0 / cercrp_max_above_1[valid_mask]
        maxeci_below_1[valid_mask] = 1.0 / cercrp_min_below_1[valid_mask]
        mineci_below_1[valid_mask] = 1.0 / cercrp_max_below_1[valid_mask]

        maxeci_above_2[valid_mask] = 1.0 / cercrp_min_above_2[valid_mask]
        mineci_above_2[valid_mask] = 1.0 / cercrp_max_above_2[valid_mask]
        maxeci_below_2[valid_mask] = 1.0 / cercrp_min_below_2[valid_mask]
        mineci_below_2[valid_mask] = 1.0 / cercrp_max_below_2[valid_mask]
        ...
        ecfor_above_1[valid_mask] = 0.0
        ecfor_below_1[valid_mask] = 0.0
        ...
        c_constrained_1[nonzero_mask] = eavail_1[nonzero_mask] / (
        ...
        ecfor_above_2[valid_mask] = 0.0
        ecfor_below_2[valid_mask] = 0.0
        ...
        c_constrained_2[nonzero_mask] = eavail_2[nonzero_mask] / (
        ...
        cprodl[valid_mask] = numpy.minimum(
            c_constrained_1[valid_mask], c_constrained_2[valid_mask]
        )
        cprodl[valid_mask] = numpy.minimum(cprodl[valid_mask], potenc[valid_mask])
        ...
        eup_above_1[valid_mask] = (
            cprodl[valid_mask] * cfrac_above[valid_mask] * ecfor_above_1[valid_mask]
        )
        eup_below_1[valid_mask] = (
            cprodl[valid_mask] * cfrac_below[valid_mask] * ecfor_below_1[valid_mask]
        )
        ...
        eup_above_2[valid_mask] = (
            cprodl[valid_mask] * cfrac_above[valid_mask] * ecfor_above_2[valid_mask]
        )
        eup_below_2[valid_mask] = (
            cprodl[valid_mask] * cfrac_below[valid_mask] * ecfor_below_2[valid_mask]
        )
        ...
        eprodl_1[valid_mask] = eup_above_1[valid_mask] + eup_below_1[valid_mask]
        Nfix_mask = (eprodl_1 - (eavail_1 + maxNfix) > 0.05) & valid_mask
        ...
        plantNfix[valid_mask] = numpy.maximum(
            eprodl_1[valid_mask] - eavail_1[valid_mask], 0.0
        )
...
def _new_growth(
    ...
    for val in ["delta_aglivc", "delta_aglive_1", "delta_aglive_2"]:
        for pft_i in pft_id_set:
            target_path = os.path.join(delta_sv_dir, "{}_{}.tif".format(val, pft_i))
            delta_agliv_dict["{}_{}".format(val, pft_i)] = target_path

    param_val_dict = {}
    # site-level parameters
    for val in ["favail_1", "favail_4", "favail_5", "favail_6", "pslsrb", "sorpmx"]:
        target_path = os.path.join(temp_dir, "{}.tif".format(val))
        param_val_dict[val] = target_path
        site_to_val = dict(
            [
    ...
    for pft_i in pft_id_set:
        ...
        if do_growth:
            ...
            shutil.copyfile(
                sv_reg["bglivc_{}_path".format(pft_i)], temp_val_dict["statv_temp"]
            )
            pygeoprocessing.raster_calculator(
                [
                    (path, 1)
            ...
            for iel in [1, 2]:
                nutrient_uptake(
                    iel,
                    int(veg_trait_table[pft_i]["nlaypg"]),
                    aligned_inputs["pft_{}".format(pft_i)],
...
def _apply_new_growth(delta_agliv_dict, pft_id_set, sv_reg):
    ...
    """
    with tempfile.NamedTemporaryFile(
        prefix="statv_temp", dir=PROCESSING_DIR
    ) as statv_temp_file:
        statv_temp_path = statv_temp_file.name
    for pft_i in pft_id_set:
        for sv in ["aglivc", "aglive_1", "aglive_2"]:
            shutil.copyfile(sv_reg["{}_{}_path".format(sv, pft_i)], statv_temp_path)
            raster_sum(
                delta_agliv_dict["delta_{}_{}".format(sv, pft_i)],
                _SV_NODATA,
                statv_temp_path,
                _SV_NODATA,
                sv_reg["{}_{}_path".format(sv, pft_i)],
...
def calc_amount_leached(minlch, amov_lyr, frlech, minerl_lyr_iel):
    ...
    linten = numpy.clip(
        1.0 - (minlch[valid_mask] - amov_lyr[valid_mask]) / minlch[valid_mask], 0.0, 1.0
    )
...
def _leach(aligned_inputs, site_param_table, month_reg, sv_reg):
    ...
    temp_dir = tempfile.mkdtemp(dir=PROCESSING_DIR)
    temp_val_dict = {}
    for val in ["fsol", "frlech_1", "frlech_2", "amount_leached", "d_statv_temp"]:
        temp_val_dict[val] = os.path.join(temp_dir, "{}.tif".format(val))
    param_val_dict = {}
    for val in [
        "sorpmx",
    ...
    sand_nodata = pygeoprocessing.get_raster_info(aligned_inputs["sand"])["nodata"][0]
    pygeoprocessing.raster_calculator(
        [
            (path, 1)
            for path in [
                param_val_dict["fleach_1"],
                param_val_dict["fleach_2"],
                aligned_inputs["sand"],
                param_val_dict["fleach_3"],
            ]
        ],
        calc_frlech_N,
        temp_val_dict["frlech_1"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    pygeoprocessing.raster_calculator(
    ...
    pygeoprocessing.raster_calculator(
        [
            ...
            for path in [
                ...
                param_val_dict["fleach_2"],
                aligned_inputs["sand"],
                param_val_dict["fleach_4"],
                temp_val_dict["fsol"],
            ]
        ],
        calc_frlech_P,
        temp_val_dict["frlech_2"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
...
def calc_iel_removed(c_consumed, iel_state_variable, c_state_variable):
    ...
    iel_consumed[valid_mask] = c_consumed[valid_mask] * (
        iel_state_variable[valid_mask] / c_state_variable[valid_mask]
    )
    return iel_consumed
...
def _grazing(
    aligned_inputs, site_param_table, month_reg, animal_trait_table, pft_id_set, sv_reg
):
    ...
    def calc_weighted_c_returned(shremc, sdremc, gfcret, pft_cover):
        ...
        weighted_c_returned[valid_mask] = (
            gfcret[valid_mask]
            * (shremc[valid_mask] + sdremc[valid_mask])
            * pft_cover[valid_mask]
        )
        return weighted_c_returned

    def calc_weighted_iel_returned_feces(shreme, sdreme, gret, fecf, pft_cover):
        ...
        weighted_iel_returned_feces = numpy.empty(shreme.shape, dtype=numpy.float32)
        weighted_iel_returned_feces[:] = _TARGET_NODATA
        weighted_iel_returned_feces[valid_mask] = (
            fecf[valid_mask]
            * gret[valid_mask]
            * (shreme[valid_mask] + sdreme[valid_mask])
            * pft_cover[valid_mask]
        )
        return weighted_iel_returned_feces

    def calc_weighted_iel_returned_urine(shreme, sdreme, gret, fecf, pft_cover):
        ...
        weighted_iel_returned_urine = numpy.empty(shreme.shape, dtype=numpy.float32)
        weighted_iel_returned_urine[:] = _TARGET_NODATA
        weighted_iel_returned_urine[valid_mask] = 0.0
        weighted_iel_returned_urine[consumed_mask] = (
            (1.0 - fecf[consumed_mask])
            * gret[consumed_mask]
    ...
    for val in [
        ...
        "sdreme",
        "weighted_iel_urine",
        "sum_weighted_C_returned",
        "sum_weighted_N_returned",
        "sum_weighted_P_returned",
    ]:
        temp_val_dict[val] = os.path.join(temp_dir, "{}.tif".format(val))
    for val in ["weighted_C_feces", "weighted_iel_feces_1", "weighted_iel_feces_2"]:
        for pft_i in pft_id_set:
            temp_val_dict["{}_{}".format(val, pft_i)] = os.path.join(
                temp_dir, "{}_{}.tif".format(val, pft_i)
            )
    ...
    for val in ["gfcret", "gret_2", "fecf_1", "fecf_2", "feclig"]:
        ...
        animal_to_val = dict(
            [
                (animal_code, float(table[val]))
                for (animal_code, table) in animal_trait_table.items()
            ]
        )
        pygeoprocessing.reclassify_raster(
            (aligned_inputs["animal_index"], 1),
            animal_to_val,
            target_path,
            gdal.GDT_Float32,
            _IC_NODATA,
    ...
    clay_nodata = pygeoprocessing.get_raster_info(aligned_inputs["clay"])["nodata"][0]
    pygeoprocessing.raster_calculator(
        [(aligned_inputs["clay"], 1)],
        calc_gret_1,
        param_val_dict["gret_1"],
        gdal.GDT_Float32,
        _IC_NODATA,
    )
    ...
    for pft_i in pft_id_set:
        ...
        shutil.copyfile(
            sv_reg["aglivc_{}_path".format(pft_i)], temp_val_dict["d_statv_temp"]
        )
        raster_difference(
            temp_val_dict["d_statv_temp"],
            _SV_NODATA,
        ...
        shutil.copyfile(
            sv_reg["stdedc_{}_path".format(pft_i)], temp_val_dict["d_statv_temp"]
        )
        raster_difference(
            temp_val_dict["d_statv_temp"],
            _SV_NODATA,
...
def calc_derived_animal_traits(input_animal_trait_table, freer_parameter_df):
    ...
    """
    input_df = pandas.DataFrame.from_dict(input_animal_trait_table, orient="index")
    animal_df = pandas.merge(input_df, freer_parameter_df, how="left", on="type")
    ...
    animal_df["SRW_modified"] = numpy.select(
        [
            animal_df["sex"] == "entire_m",
            animal_df["sex"] == "castrate",
            animal_df["sex"] == "NA",
        ],
        [
            animal_df["srw"] * 1.4,
    ...
    Nmax = animal_df["SRW_modified"] - (
        animal_df["SRW_modified"] - animal_df["birth_weight"]
    ) * numpy.exp(
        (-animal_df["CN1"] * animal_df["age"])
        / (animal_df["SRW_modified"] ** animal_df["CN2"])
    )
    N = animal_df["CN3"] * Nmax + (1.0 - animal_df["CN3"]) * animal_df["weight"]
    animal_df["Z"] = N / animal_df["SRW_modified"]  # relative size
    animal_df["ZF"] = numpy.where(
        animal_df["Z"] < animal_df["CR7"],
        1.0 + (animal_df["CR7"] - animal_df["Z"]),
        1.0,
    )
    ...
    if cycle_month_index < months_of_pregnancy:
        ...
        BW = (
            (
                ...
                + updated_trait_table["CP4"] * updated_trait_table["Z"]
            )
        ...
        updated_trait_table["W_total"] = updated_trait_table["weight"] + W_c
    elif cycle_month_index < (
        months_of_pregnancy + updated_trait_table["lactation_duration"]
    ):
        # animal is lactating
        updated_trait_table["reproductive_status_int"] = 2
        updated_trait_table["W_total"] = updated_trait_table["weight"]
        updated_trait_table["A_y"] = (cycle_month_index - months_of_pregnancy) * 30 + 1
    else:
...
def calc_max_intake(inner_animal_trait_dict):
    ...
    """
    updated_trait_table = inner_animal_trait_dict.copy()
    if updated_trait_table["BC"] > 1.0:
        CF = (
            updated_trait_table["BC"]
            * (updated_trait_table["CI20"] - updated_trait_table["BC"])
            / (updated_trait_table["CI20"] - 1.0)
        )
    else:
        CF = 1.0
    YF = 1.0  # eq 4 gives a different value for unweaned animals
    TF = 1.0  # ignore effect of temperature on intake
    LF = 1.0  # assume any lactating animals are suckling young (eq 8)
    if updated_trait_table["reproductive_status_int"] == 2:
        ...
        LA = 1.0 - updated_trait_table["CI15"] + updated_trait_table["CI15"] * BCpart
        LF = (
            1.0
            + updated_trait_table["CI19"]
            * Mi ** updated_trait_table["CI9"]
            * math.exp(updated_trait_table["CI9"] * (1 - Mi))
            * LA
        )

    updated_trait_table["max_intake"] = (
        updated_trait_table["CI1"]
        * updated_trait_table["SRW_modified"]
...
def calc_pasture_height(sv_reg, aligned_inputs, pft_id_set, processing_dir):
    ...
    def calc_weighted_biomass_kgha(cstatv, pft_cover):
        ...
        """
        valid_mask = (~numpy.isclose(cstatv, _SV_NODATA)) & (
            ~numpy.isclose(pft_cover, pft_nodata)
        )
        biomass_kgha = numpy.empty(cstatv.shape, dtype=numpy.float32)
        biomass_kgha[:] = _TARGET_NODATA
        biomass_kgha[valid_mask] = cstatv[valid_mask] * 2.5 * 10 * pft_cover[valid_mask]
        return biomass_kgha
    ...
    for pft_i in pft_id_set:
        ...
        biomass_raster_list.append(temp_val_dict["agliv_kgha_{}".format(pft_i)])
        biomass_raster_list.append(temp_val_dict["stded_kgha_{}".format(pft_i)])

    pygeoprocessing.raster_calculator(
        [(path, 1) for path in biomass_raster_list],
        calc_scale_term,
        temp_val_dict["scale_term"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
...
def calc_fraction_biomass(
    sv_reg, aligned_inputs, pft_id_set, processing_dir, total_weighted_C_path
):
    ...
    def weighted_fraction(cstatv, pft_cover, total_weighted_C):
        ...
        weighted_fraction[valid_mask] = (
            cstatv[valid_mask] * pft_cover[valid_mask] / total_weighted_C[valid_mask]
        )
    ...
    for pft_i in pft_id_set:
        ...
        target_path = os.path.join(processing_dir, "agliv_frac_bio_{}".format(pft_i))
        ...
        target_path = os.path.join(processing_dir, "stded_frac_bio_{}".format(pft_i))
...
def order_by_digestibility(sv_reg, pft_id_set, aoi_path):
    ...
    def calc_nc_ratio(cstatv_path, nstatv_path, aoi_path):
        ...
        carbon_zonal_stat_df = pandas.DataFrame.from_dict(
            pygeoprocessing.zonal_statistics((cstatv_path, 1), aoi_path), orient="index"
        )
        if carbon_zonal_stat_df["count"].sum() == 0:
            return 0
        else:
            mean_carbon = (
                carbon_zonal_stat_df["sum"].sum() / carbon_zonal_stat_df["count"].sum()
            )
        nitrogen_zonal_stat_df = pandas.DataFrame.from_dict(
            pygeoprocessing.zonal_statistics((nstatv_path, 1), aoi_path), orient="index"
        )
    ...
    sorted_list = sorted(
        [(ratio, feed_type) for (feed_type, ratio) in nc_ratio_dict.items()],
        reverse=True,
    )
...
def calc_digestibility(cstatv, nstatv, digestibility_slope, digestibility_intercept):
    ...
    ) * digestibility_slope[valid_mask] + digestibility_intercept[valid_mask]
...
def calc_relative_availability(avail_biomass, sum_previous_classes):
    ...
    """
    valid_mask = (avail_biomass != _TARGET_NODATA) & (
        sum_previous_classes != _TARGET_NODATA
    )
    relative_availability = numpy.empty(avail_biomass.shape, dtype=numpy.float32)
    relative_availability[:] = _TARGET_NODATA
    relative_availability[valid_mask] = (
        numpy.maximum(0.0, 1.0 - sum_previous_classes[valid_mask])
        * avail_biomass[valid_mask]
    )
    return relative_availability


def calc_digestibility_intake(diet_path_dict, feed_type_list, diet_digestibility_path):
    ...
    intake_path_list = [
        ...
        for feed_type in sorted(feed_type_list)
    ]
    digestibility_path_list = [
        diet_path_dict["digestibility_{}".format(feed_type)]
        for feed_type in sorted(feed_type_list)
    ]

    # sum intake across feed types
    raster_list_sum(
        intake_path_list, _TARGET_NODATA, temp_val_dict["intake_sum"], _TARGET_NODATA
    )
    ...
    for feed_type_index in range(len(intake_path_list)):
        ...
        raster_multiplication(
            intake_path_list[feed_type_index],
            _TARGET_NODATA,
            digestibility_path_list[feed_type_index],
            _TARGET_NODATA,
            target_path,
            _TARGET_NODATA,
        )
    ...
    raster_list_sum(
        weighted_digestibility_path_list,
        _TARGET_NODATA,
        temp_val_dict["digestibility_sum"],
        _TARGET_NODATA,
    )
    ...
    raster_division(
        temp_val_dict["digestibility_sum"],
        _TARGET_NODATA,
        temp_val_dict["intake_sum"],
        _TARGET_NODATA,
        diet_digestibility_path,
        _TARGET_NODATA,
    )
...
def calc_crude_protein_intake(
    ...
    for feed_type in feed_type_list:
        ...
        target_path = os.path.join(temp_dir, "weighted_cp_{}.tif".format(feed_type))
        pygeoprocessing.raster_calculator(
            [
                (path, 1)
                for path in [
...
def calc_energy_intake(total_intake, total_digestibility):
    ...
    """
    valid_mask = (total_intake != _TARGET_NODATA) & (
    ...
    energy_intake[valid_mask] = (
        17.0 * total_digestibility[valid_mask] - 2.0
...
def calc_energy_maintenance(
    ...
    nonzero_mask = (total_intake > 0) & valid_mask
    km = numpy.empty(age.shape, dtype=numpy.float32)
    km[nonzero_mask] = CK1[nonzero_mask] + CK2[nonzero_mask] * (
        energy_intake[nonzero_mask] / total_intake[nonzero_mask]
    )
    Egraze = numpy.empty(age.shape, dtype=numpy.float32)
    Egraze[nonzero_mask] = CM6[nonzero_mask] * weight[nonzero_mask] * total_intake[
        nonzero_mask
    ] * (CM7[nonzero_mask] - total_digestibility[nonzero_mask]) + (
        CM16[nonzero_mask] * 4.0 * weight[nonzero_mask]
    )
    Emetab = numpy.empty(age.shape, dtype=numpy.float32)
    Emetab[nonzero_mask] = (
        CM2[nonzero_mask]
        * weight[nonzero_mask] ** 0.75
        * numpy.maximum(
            numpy.exp(-CM3[nonzero_mask] * age[nonzero_mask]), CM4[nonzero_mask]
        )
    )
    ...
    energy_maintenance_female[nonzero_mask] = (
        Emetab[nonzero_mask] + Egraze[nonzero_mask]
    ) / km[nonzero_mask] + CM1[nonzero_mask] * energy_intake[nonzero_mask]

    energy_maintenance = energy_maintenance_female
    male_mask = (sex < 3) & (valid_mask)
...
def calc_degr_protein_intake(crude_protein_intake, total_digestibility):
    ...
    """
    valid_mask = (crude_protein_intake != _TARGET_NODATA) & (
        total_digestibility != _TARGET_NODATA
    )
    degr_protein_intake = numpy.empty(crude_protein_intake.shape, dtype=numpy.float32)
    degr_protein_intake[:] = _TARGET_NODATA
    degr_protein_intake[valid_mask] = crude_protein_intake[valid_mask] * numpy.minimum(
        0.84 * total_digestibility[valid_mask] + 0.33, 1.0
    )
...
def calc_protein_req(
    ...
    def protein_req_op(current_month):
        def _protein_req_op(
            latitude, energy_intake, energy_maintenance, CRD4, CRD5, CRD6, CRD7
        ):
            """Calculate rumen degradable protein required.
            ...
            nonzero_mask = (energy_maintenance > 0) & valid_mask
            ...
            radiation_factor[valid_mask] = 1.0 + CRD7[valid_mask] * (
                latitude[valid_mask] / 40.0
            ) * numpy.sin((2.0 * numpy.pi * day_of_year) / 365.0)
...
def revise_max_intake(
    ...
    high_intake_mask = (((energy_intake / energy_maintenance) - 1) > 0) & valid_mask
    corrected_protein_intake[high_intake_mask] = degr_protein_intake[
        high_intake_mask
    ] * (
        1.0
        - (
            CRD1[high_intake_mask]
            - CRD2[high_intake_mask] * total_digestibility[high_intake_mask]
        )
        * (energy_intake[high_intake_mask] / energy_maintenance[high_intake_mask])
        - 1.0
    )

    reduction_factor = numpy.empty(max_intake.shape, dtype=numpy.float32)
    reduction_factor[valid_mask] = 1.0
    ...
    reduction_factor[insuff_indicus_mask] = 1.0 - (
        (
            1.0
            - (
                corrected_protein_intake[insuff_indicus_mask]
                / protein_req[insuff_indicus_mask]
            )
        )
        * 0.5
    )
    ...
    reduction_factor[insuff_cross_mask] = 1.0 - (
        (
            1.0
            - (
                corrected_protein_intake[insuff_cross_mask]
                / protein_req[insuff_cross_mask]
            )
        )
        * 0.75
    )
...
def _calc_grazing_offtake(
    ...
    def calc_avail_biomass(
        cstatv, pft_cover, frac_biomass, height, ZF, CR4, CR5, CR6, CR12, CR13
    ):
        ...
        biomass[valid_mask] = cstatv[valid_mask] * 2.5 * 10 * pft_cover[valid_mask]
        relative_time = numpy.empty(cstatv.shape, dtype=numpy.float32)
        relative_time[:] = _TARGET_NODATA
        relative_time[valid_mask] = 1.0 + CR5[valid_mask] * numpy.exp(
            -(1.0 + CR13[valid_mask] * frac_biomass[valid_mask])
            * (
                CR6[valid_mask]
                * (1.0 - CR12[valid_mask] + CR12[valid_mask] * height[valid_mask])
                * ZF[valid_mask]
                * biomass[valid_mask]
            )
            ** 2
        ...
        relative_rate[valid_mask] = 1.0 - numpy.exp(
            -(1.0 + CR13[valid_mask] * frac_biomass[valid_mask])
            * CR4[valid_mask]
            * (1.0 - CR12[valid_mask] + CR12[valid_mask] * height[valid_mask])
            * ZF[valid_mask]
    ...
    def calc_relative_ingestibility(
        ...
        rel_ingestibility = numpy.empty(digestibility.shape, dtype=numpy.float32)
        rel_ingestibility[:] = _TARGET_NODATA
        rel_ingestibility[valid_mask] = 1.0 - CR3[valid_mask] * (
            CR1[valid_mask]
            - (1.0 - prop_legume[valid_mask]) * species_factor[valid_mask]
            - digestibility[valid_mask]
        )
    ...
    def calc_daily_intake(
        ...
        daily_intake = numpy.empty(proportion_legume.shape, dtype=numpy.float32)
        daily_intake[:] = _TARGET_NODATA
        daily_intake[valid_mask] = (
            maximum_intake[valid_mask]
            * relative_availability[valid_mask]
    ...
    def calc_fraction_removed(
        ...
        fgrem[valid_mask] = numpy.minimum(demand[valid_mask], max_fgrem[valid_mask])
        return fgrem
    ...
    for val in [
        ...
        for statv in ["agliv", "stded"]:
            for pft_i in pft_id_set:
                ...
                target_path = os.path.join(temp_dir, "{}.tif".format(value_string))
    ...
    # pft parameters
    for val in ["species_factor", "digestibility_slope", "digestibility_intercept"]:
        for pft_i in pft_id_set:
            target_path = os.path.join(temp_dir, "{}_{}.tif".format(val, pft_i))
            param_val_dict["{}_{}".format(val, pft_i)] = target_path
            fill_val = veg_trait_table[pft_i][val]
    ...
    pygeoprocessing.raster_calculator(
        ...
        calc_max_fraction_removed,
        temp_val_dict["max_fgrem"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    ...
    frac_biomass_dict = calc_fraction_biomass(
        sv_reg, aligned_inputs, pft_id_set, temp_dir, temp_val_dict["total_weighted_C"]
    )
    ...
    raster_list_sum(
        intake_list, _TARGET_NODATA, temp_val_dict["total_intake"], _TARGET_NODATA
    )
    calc_digestibility_intake(
        temp_val_dict, ordered_feed_types, temp_val_dict["total_digestibility"]
    )
    ...
    pygeoprocessing.raster_calculator(
        [
            ...
            for path in [
                param_val_dict["age"],
                param_val_dict["sex_int"],
                param_val_dict["W_total"],
                temp_val_dict["energy_intake"],
                temp_val_dict["total_intake"],
                temp_val_dict["total_digestibility"],
                param_val_dict["CK1"],
    ...
    pygeoprocessing.raster_calculator(
        ...
        calc_degr_protein_intake,
        temp_val_dict["degr_protein_intake"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    calc_protein_req(
...
def calc_diet_sufficiency(
    ...
    MEc_num1[pregnant_mask] = (
        ...
        * CP5[pregnant_mask]
        * (
            (1.0 - CP4[pregnant_mask] + CP4[pregnant_mask] * Z[pregnant_mask])
            * CP15[pregnant_mask]
            * SRW[pregnant_mask]
        )
        * (CP9[pregnant_mask] * CP10[pregnant_mask])
        / CP1[pregnant_mask]
    )
    MEc_num2 = numpy.empty(energy_intake.shape, dtype=numpy.float32)
    MEc_num2[pregnant_mask] = numpy.exp(
        CP10[pregnant_mask] * (1.0 - (A_foet[pregnant_mask] / CP1[pregnant_mask]))
        + CP9[pregnant_mask]
        * (
            1.0
            - numpy.exp(
                CP10[pregnant_mask]
                * (1.0 - (A_foet[pregnant_mask] / CP1[pregnant_mask]))
            )
        )
    )
    ...
    kl[lactating_mask] = CK5[lactating_mask] + CK6[lactating_mask] * (
        energy_intake[lactating_mask] / total_intake[lactating_mask]
    )
    Mm = numpy.empty(energy_intake.shape, dtype=numpy.float32)
    Mm[lactating_mask] = (A_y[lactating_mask] + CL1[lactating_mask]) / CL2[
        lactating_mask
    ]
    MPmax = numpy.empty(energy_intake.shape, dtype=numpy.float32)
    MPmax[lactating_mask] = (
        CL0[lactating_mask]
        * SRW[lactating_mask] ** 0.75
        * Z[lactating_mask]
        * BC[lactating_mask]
        * Mm[lactating_mask] ** CL3[lactating_mask]
        * numpy.exp(CL3[lactating_mask] * (1.0 - Mm[lactating_mask]))
    )
    energy_lactation[lactating_mask] = (
        MPmax[lactating_mask] / CL5[lactating_mask] * kl[lactating_mask]
    )
    protein_lactation[lactating_mask] = CL15[lactating_mask] * (
        MPmax[lactating_mask] / CL6[lactating_mask]
    )
    ...
    Dudp[valid_mask] = numpy.maximum(
        CA1[valid_mask],
        numpy.minimum(
            CA3[valid_mask] * total_crude_protein_intake[valid_mask] - CA4[valid_mask],
            CA2[valid_mask],
        ),
    )
    ...
    DPLS[valid_mask] = (
        Dudp[valid_mask]
        * (total_crude_protein_intake[valid_mask] - degr_protein_intake[valid_mask])
    ...
    diet_sufficiency[nonzero_mask] = energy_intake[nonzero_mask] / (
        energy_maintenance[nonzero_mask]
        + energy_pregnancy[nonzero_mask]
        + energy_lactation[nonzero_mask]
        + energy_wool[nonzero_mask]
    )
...
def _animal_diet_sufficiency(
    ...
    def daily_intake_from_fraction_removed(cstatv, pft_cover, animal_density, fgrem):
        ...
        daily_intake[:] = _TARGET_NODATA
        daily_intake[valid_mask] = (fgrem[valid_mask] * biomass[valid_mask]) / (
    ...
    feed_type_list = [
        "{}_{}".format(sv, pft_i) for sv in ["agliv", "stded"] for pft_i in pft_id_set
    ]
    intake_list = [
        temp_val_dict["daily_intake_{}".format(feed_type)]
        for feed_type in feed_type_list
    ...
    pygeoprocessing.raster_calculator(
        [
            ...
            for path in [
                ...
                param_val_dict["SRW_modified"],
                param_val_dict["sfw"],
                param_val_dict["age"],
                param_val_dict["Z"],
                param_val_dict["BC"],
                param_val_dict["A_foet"],
                param_val_dict["A_y"],
                param_val_dict["CK5"],
                param_val_dict["CK6"],
                param_val_dict["CK8"],
...
def calc_animal_density(adjusted_diff, sum_adjusted_diff, total_animals, pixel_area_ha):
    ...
    """
    default_mask = total_animals != _TARGET_NODATA
    valid_mask = (
...
def sum_c_to_biomass(sum_aglivc, sum_stdedc):
    ...
    """
    valid_mask = (sum_aglivc != _TARGET_NODATA) & (sum_stdedc != _TARGET_NODATA)
...
def normalize_within_grazing_areas(
    vegetation_path,
    animal_grazing_areas_shp_path,
    animal_grazing_areas_raster_path,
    target_path,
):
    ...
    def normalize_op(value, min_value, max_value):
        ...
        zero_mask = (numpy.isclose(min_value, max_value)) & valid_mask
        normalized[zero_mask] = 0.5
        nonzero_mask = (~numpy.isclose(min_value, max_value)) & valid_mask
        normalized[nonzero_mask] = (value[nonzero_mask] - min_value[nonzero_mask]) / (
    ...
    pygeoprocessing.reclassify_raster(
        (animal_grazing_areas_raster_path, 1),
        feature_to_min_val,
        temp_val_dict["min_value"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    ...
    pygeoprocessing.reclassify_raster(
        (animal_grazing_areas_raster_path, 1),
        feature_to_max_val,
        temp_val_dict["max_value"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    value_nodata = pygeoprocessing.get_raster_info(vegetation_path)["nodata"][0]
...
def calc_adjusted_diff(
    EO_normalized_path,
    biomass_potential_normalized_path,
    animal_grazing_areas_shp_path,
    animal_grazing_areas_raster_path,
    target_path,
):
    ...
    raster_difference(
        ...
        biomass_potential_normalized_path,
        _TARGET_NODATA,
        temp_val_dict["normalized_diff"],
        _IC_NODATA,
    )

    # find maximum difference within grazing area features
    zonal_stats = pygeoprocessing.zonal_statistics(
        (temp_val_dict["normalized_diff"], 1),
        animal_grazing_areas_shp_path,
        polygons_might_overlap=False,
    )
    ...
    raster_sum(
        ...
        temp_val_dict["max_diff"],
        _TARGET_NODATA,
        temp_val_dict["biomass_potential_translated"],
        _TARGET_NODATA,
    )
...
def _estimate_animal_density(
    ...
    def get_pixel_area_ha(raster_path):
        ...
        average_latitude = (bounding_box[1] + bounding_box[3]) / 2.0
        pixel_y_length_m = abs(raster_info["pixel_size"][1]) * 111139.0
        pixel_x_length_m = (
            abs(raster_info["pixel_size"][0])
            * numpy.cos(numpy.radians(average_latitude))
            * 111139.0
        )
    ...
    temp_val_dict = {}
    for val in [
        "animal_mgmt_features",
        "biomass_potential",
        "biomass_potential_normalized",
        "EO_normalized",
        "adjusted_diff",
        "total_animals",
        "sum_adjusted_diff",
        ...
        temp_val_dict[val] = os.path.join(temp_dir, "{}.tif".format(val))
    temp_val_dict["animal_mgmt_copy"] = os.path.join(temp_dir, "animal_mgmt_copy.shp")

    # generate raster of animal grazing areas
    add_shp_id_field(animal_grazing_areas_path, temp_val_dict["animal_mgmt_copy"])
    pygeoprocessing.new_raster_from_base(
        aligned_inputs["animal_index"],
    ...
    feature_to_val = dict(
        [(fid, float(table["sum"])) for fid, table in adjusted_diff_table.items()]
    )
    pygeoprocessing.reclassify_raster(
        (temp_val_dict["animal_mgmt_features"], 1),
        feature_to_val,
        temp_val_dict["sum_adjusted_diff"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
...
def _write_monthly_outputs(
    ...
    for val in [
        ...
        output_val_dict[val] = os.path.join(
            output_dir, "{}_{}_{}.tif".format(val, current_year, current_month)
        )
    ...
    pygeoprocessing.raster_calculator(
        ...
        sum_c_to_biomass,
        output_val_dict["potential_biomass"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )
    ...
    pygeoprocessing.raster_calculator(
        ...
        sum_c_to_biomass,
        output_val_dict["standing_biomass"],
        gdal.GDT_Float32,
        _TARGET_NODATA,
    )

    # density of animals inside grazing areas
    shutil.copyfile(month_reg["animal_density"], output_val_dict["animal_density"])

    # diet sufficiency
    shutil.copyfile(month_reg["diet_sufficiency"], output_val_dict["diet_sufficiency"])
...
def create_vector_copy(base_vector_path, target_vector_path):
    ...
    target_vector = None  # noqa: F841 seemingly uncessary but gdal seems to like it.
...
def aggregate_and_pickle_results(output_dir, grazing_areas_path):
    ...
    def dump_zonal_stats(raster_prefix, pickle_path):
        ...
        raster_path_list = [
            ...
            if f.startswith(raster_prefix) and f.endswith(".tif")
        ]
        df_list = []
        for raster_path in raster_path_list:
            ...
            zonal_df = pandas.DataFrame(
                {
                    "fid": [key for key, value in sorted(zonal_stat_dict.items())],
                    "sum": [
                        value["sum"] for key, value in sorted(zonal_stat_dict.items())
                    ],
                    "count": [
                        value["count"] for key, value in sorted(zonal_stat_dict.items())
                    ],
                }
            )
    ...
    diet_suff_pickle_path = os.path.join(PROCESSING_DIR, "diet_sufficiency.pickle")
...
def _add_fields_to_shapefile(field_pickle_map, field_header_order, target_vector_path):
    ...
    """
    target_vector = gdal.OpenEx(target_vector_path, gdal.OF_VECTOR | gdal.GA_Update)
    ...
    for field_name in field_header_order:
        ...
        with open(field_pickle_map[field_name], "rb") as pickle_file:
            field_summaries[field_name] = pickle.load(pickle_file)  # noqa: S301
...
def calc_mean_animal_density(output_dir, target_path):
    ...
    def raster_mean_op(*raster_list):
        ...
        divide_mask = (sum_of_rasters > 0) & (num_observations > 0) & valid_mask

        mean_of_rasters = numpy.empty(sum_of_rasters.shape, dtype=numpy.float32)
    ...
    raster_path_list = [
        os.path.join(output_dir, f)
        for f in os.listdir(output_dir)
        if f.startswith("animal_density_") and f.endswith(".tif")
    ]
...
def validate(args, limit_to=None):
    ...
    if no_value_list:
        validation_error_list.append((no_value_list, "parameter has no value"))

    # check that spatial input files are the correct types
    # and are in geographic coordinates
    with capture_gdal_logging():
        for key, filetype, filetype_string in spatial_file_list:
            ...
            if spatial_file is None:
                validation_error_list.append(([key], "Must be a %s" % filetype_string))
                continue

            else:
                if filetype_string == "vector":
                    input_proj = pygeoprocessing.get_vector_info(args[key])[
                        "projection"
                    ]
                else:
                    input_proj = pygeoprocessing.get_raster_info(args[key])[
                        "projection"
                    ]
...
def get_rainfall_filepath(rainfall_src, year, month):
    dir_name = os.path.dirname(rainfall_src)
    src = f"chirps-v2.0.{year}.{str(month).zfill(2)}.tif"
    return os.path.join(dir_name, src)
