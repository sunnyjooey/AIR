...
from models.crop_model.io import find_vector_coords
from models.crop_model.soil_handler import (
    calculateICLayerData,
    findSoilProfile,
    readSoilLayers,
)

# import pythia.template
from models.crop_model.util import to_julian_date
...
def auto_planting_window(k, run, context, _):
    ...
    last = first + td
    return {
        "pdate": to_julian_date(first),
        "pfrst": to_julian_date(first),
        "plast": to_julian_date(last),
    }
...
def generate_ic_layers(k, run, context, _):
    ...
    if args[0].startswith("$"):
        ...
        profile = args[0]
    soil_file = findSoilProfile(context[profile], context["soilFiles"])
    layers = readSoilLayers(context[profile], soil_file)
    calculated_layers = calculateICLayerData(layers, run)
