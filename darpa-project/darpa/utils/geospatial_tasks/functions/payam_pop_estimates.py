...
def estimate_payam_level_population(
    geoshape_file,
    pop_rates_file,
    ref_proj,
    fname_out="payam_level_pop_out.tif",
    year=2017,
    no_data_val=-9999,
    birth_rate_fct=0.0,
    death_rate_fct=0.0,
    ...
    """ Estimate payam level population based on latest census data and growth rate.
    ...
    geoshape_file : ESRI Shape file (*.shp)
        The geoshape_file has the payam boundaries for South Sudan. These boundaries are used to
        rasterize the various population data (2008 census, CBR, CDR, and migration).

    pop_rates_file: CSV file (*.csv)
        This file has the population values by payam (2008 census, CBR, CDR, and migration)
    ...
    pop_data = pop_data.drop(["Region", "State", "County", "Payam"], axis=1)
    pop_data = shp_data.merge(pop_data, on=["C_Payam"], how="inner")

    # -------------------------------------
    # Calculate Population from 2008 Census
    # data and Crude Birth Rates (CBR) and
    # Crude Death Rates (CDR). This population  equation growth rate
    # is usually logarithmic
    # -------------------------------------
...
def create_payam_cohorts(
    geoshape_file,
    pop_rates_file,
    age_gender_file,
    ref_proj,
    no_data_val=-9999,
    fname_out="payam_level_cohort_out.tif",
    year=2017,
):

    """Rasterize payam level cohorts.

    This functions rasterizes payam level cohorts (ratios of age and gender) breakdown of total
