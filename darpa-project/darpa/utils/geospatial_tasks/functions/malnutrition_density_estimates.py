...
def estimate_county_level_malnutrition(
    ...
    with rasterio.open(fname_out, "w", **rast_meta) as out_raster:
        ...
        burned_data_gam = features.rasterize(
            shapes=shapes_gam, fill=0, out=out_array1, transform=out_raster.transform
        )

        burned_data_sam = features.rasterize(
            shapes=shapes_sam, fill=0, out=out_array2, transform=out_raster.transform
        )

        out_raster.write_band(1, burned_data_gam)
        out_raster.write_band(2, burned_data_sam)
...
"""
def create_county_cohorts(geoshape_file, pop_rates_file, age_gender_file, ref_proj,
                          no_data_val=-9999, fname_out='county_level_cohort_out.tif', year=2017):
